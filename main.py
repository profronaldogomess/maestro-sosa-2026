...
# ==============================================================================
# MÓDULO: LABORATÓRIO DE PRODUÇÃO DIDÁTICA (CRIADOR DE AULAS) - V2026.PRO_EXECUTIVE
# (INÍCIO/MEIO/FIM, LETRAMENTO BNCC, PONTE DE INTELIGÊNCIA PONTO ID E PEI ON-DEMAND)
# ==============================================================================
elif menu == "🧪 Criador de Aulas":
    st.title("Laboratório de Produção Didática")
    st.caption("Desenvolvimento de aulas expositivas e práticas, projetos interdisciplinares e adaptações inclusivas PEI sob demanda.")
    st.markdown("---")
    ...
    def extrair_memoria_aulas_trimestre(turma_ou_ano, trimestre):
        ...
        if not df_planos.empty:
            ...
            for _, r_p in planos_rel.iterrows():
                base_txt = ai.extrair_tag(str(r_p.get('PLANO_TEXTO', '')), "BASE_DIDATICA")
                if base_txt and "Livro" in base_txt:
                    paginas_livro.add(base_txt)
                    
        memoria_txt = f"--- MEMÓRIA DAS AULAS MINISTRADAS ({turma_ou_ano} / {trimestre}) ---\n"
        memoria_txt += f"• Aulas Concluídas no Diário: {len(df_reg_trim)}\n"
        if paginas_livro:
            memoria_txt += f"• Páginas do Livro Didático: {', '.join(sorted(list(paginas_livro)))}\n"
        memoria_txt += "• Conteúdos Ministrados:\n  - " + "\n  - ".join(conteudos_usados) + "\n\n"
        
        return memoria_txt, conteudos_usados

    tab_forja, tab_acervo_lab = st.tabs(["Produção de Materiais", "Acervo Digital & Hub de Produção"])

    # ==============================================================================
    # ABA 1: PRODUÇÃO DE MATERIAIS
    # ==============================================================================
    with tab_forja:
        if "lab_temp" in st.session_state:
            ...
            def renderizar_mesa_lapidacao_lab():
                ...
                s_id = st.session_state.get("sosa_id_atual", "MATERIAL")
                meta = st.session_state.get("lab_meta", {})
                st.info(f"Material em Edição: **{s_id}**")

                with st.container(border=True):
                    st.markdown("#### Coautoria & Refinamento com IA")
                    if "chat_history_lab" not in st.session_state:
                        st.session_state.chat_history_lab = [{"role": "assistant", "avatar": "🤖", "content": "Roteiro pedagógico estruturado. Como deseja refinar o material?"}]
                    
                    chat_container_lab = st.container(height=180)
                    with chat_container_lab:
                        for msg in st.session_state.chat_history_lab:
                            with st.chat_message(msg["role"], avatar=msg["avatar"]):
                                st.markdown(msg["content"])
                    
                    if cmd_refine_lab := st.chat_input("Instrução de ajuste (ex: 'Adicione uma contextualização sobre tecnologia no início')", key=f"chat_lab_ref_{v}"):
                        ...
                        with chat_container_lab:
                            ...
                            with st.chat_message("assistant", avatar="🤖"):
                                with st.spinner("Refinando material..."):
                                    ...
                                    msg_chat = ai.extrair_tag(resultado_refino, "MENSAGEM_CHAT") or "Ajuste concluído com sucesso."
                    ...
                    if st.button("Descartar Edição e Retornar", use_container_width=True, key=f"btn_disc_lab_{v}"): 
                        reset_laboratorio()
                
                st.markdown("---")
                modo_leitura = st.toggle("Renderizar Visualização Matemática", value=False, key=f"read_mode_lab_{v}")
                ...
                t_prof, t_alu, t_gab, t_pei_tab, t_img_tab, t_sync = st.tabs(["Guia Docente (Lousa)", "Folha do Estudante", "Gabarito Comentado", "Adaptação PEI", "Ilustrações", "Custódia Digital"])
                
                with t_prof: 
                    if modo_leitura: st.markdown(preparar_para_leitura(val_prof))
                    else: ed_prof = st.text_area("Roteiro Docente:", val_prof, height=340, key=f"ed_prof_reg_{v}")
                
                with t_alu: 
                    if modo_leitura: st.markdown(preparar_para_leitura(val_alu))
                    else: ed_alu = st.text_area("Folha do Estudante:", val_alu, height=340, key=f"ed_alu_reg_{v}")
                
                with t_gab: 
                    if modo_leitura: st.markdown(preparar_para_leitura(val_gab))
                    else: ed_gab = st.text_area("Resoluções Comentadas:", val_gab, height=200, key=f"ed_res_reg_{v}")
                
                with t_pei_tab: 
                    st.markdown("**PEI Nível 1 & Nível 2 (Apoio Direto):**")
                    ed_pei1 = st.text_area("Nível 1/2:", val_pei1, height=180, key=f"ed_pei1_reg_{v}")
                    st.markdown("**PEI Nível 3 (Bento Cards no Papel):**")
                    ed_pei3 = st.text_area("Nível 3:", val_pei3, height=180, key=f"ed_pei3_reg_{v}")
                
                with t_img_tab: 
                    ed_img = st.text_area("Diretrizes de Ilustração Técnica:", val_img, height=140, key=f"ed_img_reg_{v}")

                with t_sync:
                    st.markdown("#### Sincronização & Custódia no Google Drive")
                    if st.button("Sincronizar Acervo no Google Drive", use_container_width=True, type="primary", key=f"btn_triple_{v}"):
                        with st.status("Sincronizando arquivos no Google Drive...") as status:
                            ...
                            status.write("Gerando Folha do Estudante DOCX...")
                            ...
                            link_pei1 = "N/A"
                            if ed_pei1:
                                status.write("Gerando Caderno PEI N1 DOCX...")
                            ...
                            link_pei3 = "N/A"
                            if ed_pei3:
                                status.write("Gerando Caderno PEI N3 DOCX...")
                            ...
                            status.write("Gerando Guia Docente DOCX...")
                            ...
                            status.update(label="Materiais sincronizados com sucesso!", state="complete")
                            st.balloons(); time.sleep(0.8); reset_laboratorio()

            renderizar_mesa_lapidacao_lab()

        else:
            if fa['fase'] == 1:
                st.markdown("### Configuração da Linha de Produção")
                
                tipo_criacao = st.segmented_control(
                    "Tipo de Material a Produzir:", 
                    ["Aula Regular (Teoria & Prática)", "Projeto Interdisciplinar", "Recomposição & Lista de Exercícios"], 
                    default="Aula Regular (Teoria & Prática)",
                    key=f"lab_pills_tipo_{v}"
                )
                
                if "Aula Regular" in tipo_criacao:
                    fa['tipo_material'] = 'AULA'
                    with st.container(border=True):
                        st.markdown("#### 1. Herança de Roteiro do Planejamento & Livro Didático")
                        sobrescrever_lab_flag = st.toggle("Exibir semanas concluídas (Permitir Edição)", value=False, key=f"tog_sobrescrever_lab_{v}")
                        ...
                        if planos_ano.empty:
                            st.success(f"Nenhum material pendente no Criador de Aulas para o {ano_lab}º Ano.")
                            st.info("Semanas concluídas ou ministradas presencialmente estão arquivadas. Para editar, ative a opção acima.")
                        else:
                            semanas_opcoes = planos_ano["SEMANA"].unique().tolist()
                            
                            sem_lab = c2.selectbox("Semana Pendente:", semanas_opcoes, key=f"prod_sem_{v}")
                            ...
                            conteudos_espec = ai.extrair_tag(plano_txt, "CONTEUDOS_ESPECIFICOS")
                            objetivos_ensino = ai.extrair_tag(plano_txt, "OBJETIVOS_ENSINO")
                            estrategia_pei_plano = ai.extrair_tag(plano_txt, "ESTRATEGIA_DUA_PEI")
                            base_herdada = ai.extrair_tag(plano_txt, "BASE_DIDATICA")

                            a_geradas_sem = df_aulas[(df_aulas['ANO'].astype(str).str.contains(str(ano_lab))) & (df_aulas['SEMANA_REF'] == sem_lab)]['TIPO_MATERIAL'].astype(str).tolist() if not df_aulas.empty and 'ANO' in df_aulas.columns and 'SEMANA_REF' in df_aulas.columns else []
                            ...
                            qtd_q_prod = c_c2.slider("Exercícios na Folha:", 1, 15, 5, key=f"sld_qtd_q_{v}")
                            ...
                            with st.container(border=True):
                                st.markdown(f"##### Diretrizes Pedagógicas ({sem_lab} — {ano_lab}º Ano)")
                                c_dna1, c_dna2 = st.columns(2)
                                c_dna1.markdown(f"**Objeto de Conhecimento:** `{obj_conhecimento if obj_conhecimento else 'Geral'}`")
                                c_dna1.markdown(f"**Habilidade BNCC:** `{hab_bncc if hab_bncc else 'EF06MA01'}`")
                                c_dna2.markdown(f"**Conteúdos:** *{conteudos_espec[:120] if conteudos_espec else 'Definidos no plano'}...*")
                                c_dna2.markdown(f"**Objetivo:** *{objetivos_ensino[:100] if objetivos_ensino else 'Desenvolvimento de competências'}...*")
                                
                                st.divider()
                                st.markdown(f"**Roteiro da Aula ({aula_alvo_prod}):**")
                                st.info(roteiro_especifico if roteiro_especifico else "Roteiro não especificado.")

                            st.markdown("##### Ancoragem no Livro Didático")
                            
                            col_p1, col_p2 = st.columns(2)
                            pags_teo_lab = col_p1.text_input("Páginas de Teoria:", value=sugestao_pags_teo, placeholder="Ex: 198, 199, 201", key=f"pags_teo_lab_{v}")
                            pags_ex_lab = col_p2.text_input("Páginas de Exercícios:", value=sugestao_pags_ex, placeholder="Ex: 202", key=f"pags_ex_lab_{v}")
                            ...
                            if "https" in base_herdada or "drive.google.com" in base_herdada:
                                ...
                                if list_p_teo or list_p_ex:
                                    with st.spinner("Fatiando páginas do livro didático..."):
                            ...
                            recorte_exercicios_livro = st.text_area(
                                "Instruções Complementares do Professor (Opcional):",
                                placeholder="Insira diretrizes complementares ou exercícios autorais...",
                                height=85,
                            ...
                            pacote_recorte_aula = (
                                ...
                                f"CONTEÚDOS ESPECÍFICOS: {conteudos_espec}\n"
                                f"HABILIDADES BNCC: {hab_bncc}\n"
                                f"COMPETÊNCIAS ESPECÍFICAS: {comp_foco}\n"
                                f"OBJETIVOS: {objetivos_ensino}\n\n"
                                f"--- ROTEIRO DA AULA ({aula_alvo_prod}) ---\n{roteiro_especifico}\n\n"
                            )
                            
                            if txt_lab_teo_ext: pacote_recorte_aula += f"--- PÁGINAS DE TEORIA ---\n{txt_lab_teo_ext}\n\n"
                            if txt_lab_ex_ext: pacote_recorte_aula += f"--- PÁGINAS DE EXERCÍCIOS ---\n{txt_lab_ex_ext}\n\n"
                            if recorte_exercicios_livro.strip(): pacote_recorte_aula += f"--- TEXTO COMPLEMENTAR ---\n{recorte_exercicios_livro.strip()}\n\n"

                            links_web_aula = st.text_area("Referências Adicionais (URLs no Drive/Web):", value=base_herdada if "https" in base_herdada else "", key=f"links_web_lab_{v}")

                            c_f_btn1, c_f_btn2, c_f_btn3 = st.columns([1.5, 1.5, 1.2])

                            if c_f_btn1.button("Iniciar Produção Guiada", use_container_width=True, key=f"btn_guiada_lab_{v}"):
                                fa['info'] = {
                                    "ano": ano_lab, "semana_ref": sem_lab, "aula_alvo": aula_alvo_prod,
                                    "roteiro": roteiro_especifico, "habilidade": hab_bncc,
                                    "objetivos": objetivos_ensino, "base": base_herdada,
                            ...
                            if c_f_btn2.button("Executar Forja Direta", use_container_width=True, type="primary", key=f"btn_turbo_lab_{v}"):
                                with st.status("Executando produção direta com Gemini 3.7 Flash...", expanded=True) as status_turbo:
                                    info_turbo = {
                                        "ano": ano_lab, "semana_ref": sem_lab, "aula_alvo": aula_alvo_prod,
                                        "roteiro": roteiro_especifico, "habilidade": hab_bncc,
                                        "objetivos": objetivos_ensino, "base": base_herdada,
                                    ...
                                    status_turbo.write("1/3 Estruturando Guia Docente...")
                                    ...
                                    status_turbo.write("2/3 Estruturando Folha do Estudante & Gabarito...")
                                    ...
                                    fa['reg_gab'] = ai.extrair_tag(res_ex, "GABARITO") or "Gabarito formatado."

                                    status_turbo.write("3/3 Estruturando Adaptações Inclusivas PEI...")
                                    ...
                                    fa['pei_3'] = ai.extrair_tag(res_pei, "PEI_NIVEL_3") or "PEI N3 formatado."

                                    fa['info'] = info_turbo
                                    fa['fase'] = 5
                                    status_turbo.update(label="Produção concluída com sucesso!", state="complete")
                                    st.balloons(); time.sleep(0.8); st.rerun()

                            with c_f_btn3.popover("Registrar Aula Presencial"):
                                st.caption("Registre a aula ministrada com Livro Didático sem gerar novos arquivos.")
                                det_livro_input = st.text_input("Detalhamento:", placeholder="Ex: Livro Didático Págs. 198 a 202", key=f"inp_baixa_lab_{v}")
                                dt_baixa_lab = st.date_input("Data de Aplicação:", date.today(), format="DD/MM/YYYY", key=f"dt_baixa_lab_{v}")
                                
                                if st.button("Confirmar Registro Presencial", type="primary", use_container_width=True, key=f"btn_baixa_direct_lab_{v}"):
                                    ...
                                    st.success(f"Aula de {sem_lab} registrada no diário com sucesso!")
                                    time.sleep(0.8); st.rerun()

                elif "Projeto" in tipo_criacao:
                    fa['tipo_material'] = 'PROJETO'
                    with st.container(border=True):
                        st.markdown("#### Parâmetros do Projeto Interdisciplinar")
                        c1, c2, c3 = st.columns([2, 1, 1])
                        natureza_p = c1.selectbox("Abordagem:", ["Semanário Temático", "Projeto de Identidade Local (Itabuna)", "Investigação Científica"], key=f"proj_nat_{v}")
                        ano_t = c2.selectbox("Série Alvo:", [6, 7, 8, 9], key=f"proj_ano_{v}")
                        modo_t = c3.selectbox("Modo de Execução:", ["Individual", "Equipes"], key=f"proj_modo_{v}")

                        tema_t = st.text_input("Tema Central do Projeto:", placeholder="Ex: Matemática e Sustentabilidade na Lavoura Cacaueira", key=f"proj_tema_{v}")
                        valor_t = st.number_input("Pontuação (0.0 a 10.0):", 0.0, 10.0, 2.0, key=f"proj_val_{v}")
                        
                        df_cur_t = df_curriculo[df_curriculo["ANO"].astype(str).str.contains(str(ano_t))] if not df_curriculo.empty else pd.DataFrame()
                        conts_t = st.multiselect("Conteúdos da Matriz:", sorted(df_cur_t["CONTEUDO_ESPECIFICO"].unique().tolist()) if not df_cur_t.empty and "CONTEUDO_ESPECIFICO" in df_cur_t.columns else [], key=f"proj_conts_{v}")
                        instr_extra_p = st.text_area("Diretrizes Específicas de Pesquisa:", key=f"proj_instr_{v}")

                        if st.button("Estruturar Roteiro do Projeto", use_container_width=True, type="primary", key=f"btn_gen_proj_{v}"):
                            if not tema_t: st.error("Informe o tema central do projeto.")
                            else:
                                with st.spinner("Estruturando projeto de pesquisa..."):
                    ...
                    with st.container(border=True):
                        st.markdown("#### Parâmetros da Lista de Recomposição")
                        c1, c2 = st.columns([1, 2])
                        ...
                        puxar_memoria_lista = st.toggle("Carregar Histórico de Aulas do Diário de Bordo", value=True, key=f"tog_mem_list_{v}")
                        contexto_aulas = ""
                        ...
                        if puxar_memoria_lista:
                            ...
                            if memoria_lista_txt:
                                st.success("Memória de conteúdos ministrados carregada com sucesso!")
                                contexto_aulas = memoria_lista_txt
                            else:
                                st.info("Nenhuma aula registrada no diário para este trimestre ainda.")
                        ...
                        aulas_sel = st.multiselect("Temas Adicionais do Acervo:", aulas_opcoes, key=f"ms_aulas_lab_{v}")
                        
                        for a_n in aulas_sel:
                            match_sel = df_aulas_ano[df_aulas_ano['TIPO_MATERIAL'] == a_n]
                            if not match_sel.empty:
                                contexto_aulas += str(match_sel.iloc[0].get('CONTEUDO', '')) + "\n"

                        c_q1, c_q2 = st.columns(2)
                        qtd_total_list = c_q1.slider("Quantidade de Questões:", 5, 30, 10, step=5, key=f"sld_qtd_list_{v}")
                        exigir_graficos = c_q2.checkbox("Priorizar suporte visual em questões elegíveis", value=True, key=f"chk_graf_list_{v}")
                        
                        if st.button("Iniciar Montagem da Lista", type="primary", use_container_width=True, key=f"btn_init_list_{v}"):
                            if not contexto_aulas.strip() and not aulas_sel:
                                st.error("Selecione ao menos um tema ou ative o resgate de memória.")
                            else:
                                with st.spinner("Estruturando mapa de questões..."):
                                    s_id_l = util.gerar_sosa_id("LISTA", ano_alvo, trim_lista.split()[0])
                                    nome_elite_c = f"{ano_alvo}º Ano - Lista de Exercícios - {s_id_l}"
                                    ...
                                    for idx_item in range(1, qtd_total_list + 1):
                                        ...
                                        mapa_itens.append({
                                            'q': idx_item,
                                            'tema': f"Tópico Curricular (Item {idx_item})",
                ...
                if fa.get('tipo_material') == 'AULA':
                    st.markdown("### Fase 2: Roteiro Docente & Mediação de Lousa")
                    
                    if not fa.get('teoria') or len(str(fa.get('teoria')).strip()) < 20:
                        with st.spinner("Estruturando explicação didática..."):
                            prompt_teoria = (
                                ...
                                f"RESPONDA EXCLUSIVAMENTE DENTRO DA TAG [PROFESSOR]"
                            )
                            ...
                            if len(teoria_limpa) > 30:
                                ...
                                st.error("A IA não retornou o texto completo. Tente novamente.")
                                if st.button("Tentar Novamente", key=f"btn_retry_teo_f2_{v}", type="primary"):
                                    st.rerun()
                    else:
                        with st.container(border=True):
                            modo_leitura = st.toggle("Renderizar Visualização Matemática", value=True, key=f"read_mode_f2_{v}")
                            if modo_leitura: 
                                st.markdown(preparar_para_leitura(fa['teoria']))
                            else: 
                                fa['teoria'] = st.text_area("Edição do Roteiro Docente:", value=fa['teoria'], height=320, key=f"ed_teo_manual_{v}")
                        
                        inst_t = st.text_input("Instrução de Ajuste:", key=f"inst_t_{v}")
                        c_b1, c_b2 = st.columns([2, 1])
                        
                        if c_b1.button("Aprovar Roteiro e Avançar", type="primary", use_container_width=True, key=f"btn_apr_teo_f2_{v}"):
                            fa['fase'] = 3
                            st.rerun()
                            
                        if c_b2.button("Regerar com Ajustes", use_container_width=True, key=f"btn_regerar_teo_f2_{v}"):
                            with st.spinner("Regerando roteiro..."):
                                prompt_teoria = f"SÉRIE: {fa['info']['ano']}º Ano.\nASSUNTO: {fa['info']['aula_alvo']}.\nAJUSTE: {inst_t}\nTEORIA ANTERIOR:\n{fa['teoria']}\nRESPONDA DENTRO DA TAG [PROFESSOR]"
                ...
                st.markdown("### Fase 3: Folha do Estudante & Gabarito")
                
                if not fa['reg_q']:
                    with st.spinner("Gerando exercícios contextualizados e resoluções..."):
                    ...
                    modo_leitura_ex = st.toggle("Renderizar Visualização Matemática", value=True, key=f"tog_ex_{v}")
                    t_q, t_g = st.tabs(["Folha do Estudante", "Resoluções Comentadas"])
                    with t_q:
                        if modo_leitura_ex: st.markdown(preparar_para_leitura(fa['reg_q']))
                        else: fa['reg_q'] = st.text_area("Exercícios:", value=fa['reg_q'], height=280, key=f"ed_reg_q_{v}")
                    with t_g:
                        if modo_leitura_ex: st.markdown(preparar_para_leitura(fa['reg_gab']))
                        else: fa['reg_gab'] = st.text_area("Gabarito Comentado:", value=fa['reg_gab'], height=180, key=f"ed_reg_gab_{v}")
                    
                    if st.button("Aprovar Exercícios e Avançar para PEI", type="primary", use_container_width=True, key=f"btn_f3_to_f4_{v}"):
                        fa['fase'] = 4; st.rerun()

            elif fa['fase'] == 4:
                st.markdown("### Fase 4: Adaptações Inclusivas (PEI sob Demanda)")
                st.caption("Selecione os níveis necessários para a turma para otimização de processamento:")

                with st.container(border=True):
                    niveis_lab_sel = st.segmented_control(
                        "Níveis de Acessibilidade Necessários:",
                        ["PEI Nível 1 (Apoio Leve)", "PEI Nível 3 (Suporte no Papel)"],
                        default="PEI Nível 1 (Apoio Leve)",
                        key=f"pills_pei_lab_{v}"
                    )

                pede_lab_n1 = "Nível 1" in str(niveis_lab_sel)
                pede_lab_n3 = "Nível 3" in str(niveis_lab_sel)

                if st.button("Gerar Adaptações Selecionadas", type="primary", use_container_width=True, key=f"btn_gen_pei_f4_{v}"):
                    with st.spinner("Gerando adaptações inclusivas ancoradas no tema da aula..."):
                        prompt_pei = f"Adapte as questões abaixo para PEI N1 e PEI N3 (Ancorado no Tema):\n{fa['reg_q']}\n\nTEORIA:\n{fa['teoria']}\nUSE AS TAGS [PEI_NIVEL_1], [PEI_NIVEL_3] e [GABARITO_PEI]"
                        res_pei = ai.gerar_ia("FORJA_AULA_PEI", prompt_pei)
                        
                        if pede_lab_n1: fa['pei_1'] = ai.extrair_tag(res_pei, "PEI_NIVEL_1") or res_pei
                        else: fa['pei_1'] = ""
                        
                        if pede_lab_n3: fa['pei_3'] = ai.extrair_tag(res_pei, "PEI_NIVEL_3") or "PEI N3 formatado."
                        else: fa['pei_3'] = ""
                        
                        fa['pei_gab'] = ai.extrair_tag(res_pei, "GABARITO_PEI") or "Gabarito formatado."
                        st.rerun()

                if fa.get('pei_1') or fa.get('pei_3'):
                    modo_leitura_pei = st.toggle("Renderizar Visualização Matemática", value=True, key=f"tog_pei_view_lab_{v}")
                    t_p1, t_p3 = st.tabs(["PEI Nível 1 (Apoio Leve)", "PEI Nível 3 (Bento Cards no Papel)"])
                    
                    with t_p1: 
                        if modo_leitura_pei: st.markdown(preparar_para_leitura(fa['pei_1']))
                        else: fa['pei_1'] = st.text_area("Nível 1:", value=fa['pei_1'], height=220, key=f"ed_p1_area_f4_{v}")
                        
                    with t_p3: 
                        if modo_leitura_pei: st.markdown(preparar_para_leitura(fa['pei_3']))
                        else: fa['pei_3'] = st.text_area("Nível 3:", value=fa['pei_3'], height=220, key=f"ed_p3_area_f4_{v}")
                    
                    if st.button("Aprovar Adaptações e Compilar", type="primary", use_container_width=True, key=f"btn_f4_to_f5_{v}"):
                        fa['fase'] = 5; st.rerun()

            elif fa['fase'] == 5:
                st.markdown("### Fase 5: Compilação & Sincronização no Google Drive")
                nome_sugerido = util.gerar_nome_material_elite(fa['info']['ano'], fa['info']['aula_alvo'], fa['info']['semana_ref'])
                nome_arq = st.text_input("Identificador do Material no Drive:", value=nome_sugerido, key=f"nome_arq_f5_{v}")
                
                if st.button("Finalizar e Sincronizar no Google Drive", type="primary", use_container_width=True, key=f"btn_finalizar_f5_{v}"):
                    with st.status("Gerando documentos oficiais DOCX e enviando para o Drive...", expanded=True) as status:
                        trim_real = fa['info'].get('trimestre', 'I Trimestre')
                        info_doc = {"ano": f"{fa['info']['ano']}º", "trimestre": trim_real, "semana": fa['info']['semana_ref']}

                        status.write("Compilando Folha do Estudante...")
                        ...
                        link_pei1 = "N/A"
                        if fa.get('pei_1'):
                            status.write("Compilando Caderno PEI Nível 1...")
                        ...
                        link_pei3 = "N/A"
                        if fa.get('pei_3'):
                            status.write("Compilando Caderno PEI Nível 3...")
                        ...
                        status.write("Compilando Guia Docente...")
                        ...
                        status.update(label="Materiais sincronizados no Google Drive!", state="complete")
                        st.balloons(); time.sleep(0.8); reset_laboratorio()
    ...
    with tab_acervo_lab:
        ...
        st.caption("Acesso rápido a materiais estruturados, reindexação e conclusão de pendências.")
        ...
        if not df_planos.empty:
            ...
            if not planos_ativos_hub.empty:
                st.markdown("#### Planos Pendentes no Hub de Produção")
                for _, r_hub in planos_ativos_hub.iterrows():
                    with st.container(border=True):
                        c_h1, c_h2, c_h3, c_h4 = st.columns([2, 1.2, 1.2, 1])
                        ...
                        with c_h3.popover("Registrar Aula Presencial"):
                            st.caption("Registre a aula ministrada presencialmente com Livro Didático.")
                            txt_obs_manual = st.text_input("Detalhamento:", placeholder="Ex: Livro Didático Págs. 45 a 48", key=f"txt_man_obs_{r_hub.name}")
                            
                            if st.button("Confirmar Registro Presencial", type="primary", key=f"btn_conf_man_{r_hub.name}"):
                                nome_aula_man = f"{r_hub['ANO']} - Aula Livro ({r_hub['SEMANA']})"
                                txt_conteudo_man = f"[PROFESSOR]\nAULA PRESENCIAL / LIVRO DIDÁTICO\nDetalhamento: {txt_obs_manual}\n\n[ALUNO]\nResolução dos exercícios do Livro Didático.\n\n--- LINKS ---\nRegular(N/A)"
                                ...
                                st.success("Aula presencial arquivada no diário com sucesso!")
                                time.sleep(0.6); st.rerun()
        ...
        st.markdown("---")
        st.markdown("#### Acervo de Materiais Didáticos Estruturados")
        
        with st.container(border=True):
            c_fl1, c_fl2 = st.columns(2)
            f_tri_aula = c_fl1.segmented_control("Trimestre:", ["Todos", "I Trimestre", "II Trimestre", "III Trimestre"], default="Todos", key=f"f_tri_aulas_lab_{v}")
            f_ano_aula = c_fl2.segmented_control("Série:", ["Todas", "6º", "7º", "8º", "9º"], default="Todas", key=f"f_ano_aulas_lab_{v}")
        ...
        if not df_aulas.empty:
            ...
            for _, row in df_m_acervo.iterrows():
                with st.container(border=True):
                    ...
                    if l_alu and "http" in str(l_alu): c_b1.link_button("Estudante", str(l_alu), use_container_width=True)
                    else: c_b1.caption("Presencial/Livro")
                    ...
                    if l_prof and "http" in str(l_prof): c_b4.link_button("Guia Docente", str(l_prof), use_container_width=True)
                    else: c_b4.caption("Sem Guia")
                    
                    if c_b5.button("Refinar", key=f"ref_ac_{row.name}_{v}", use_container_width=True):
                    ...
                    if c_b6.button("Excluir", key=f"del_ac_{row.name}_{v}", use_container_width=True):
                        if db.excluir_registro_com_drive("DB_AULAS_PRONTAS", identificador): st.rerun()

                    with st.expander("Recompilar Material no Padrão Exporter 2026", expanded=False):
                        st.caption("Recompilação automática aplicando formatação limpa de LaTeX e tabelas oficiais no Google Drive.")
                        
                        if st.button("Recompilar Material", type="primary", use_container_width=True, key=f"btn_recompila_{row.name}_{v}"):
                            with st.status("Recompilando material no padrão oficial...", expanded=True) as status_rec:
                                ...
                                status_rec.write("Recompilando Folha do Estudante DOCX...")
                                ...
                                link_pei1_r = "N/A"
                                if ed_pei1_r:
                                    status_rec.write("Recompilando PEI Nível 1 DOCX...")
                                ...
                                link_pei3_r = "N/A"
                                if ed_pei3_r:
                                    status_rec.write("Recompilando PEI Nível 3 DOCX...")
                                ...
                                status_rec.write("Recompilando Guia Docente DOCX...")
                                ...
                                status_rec.update(label="Material recompilado com sucesso!", state="complete")
                                st.balloons(); time.sleep(0.8); st.rerun()
...
# ==============================================================================
# MÓDULO: CENTRAL DE AVALIAÇÕES - V2026.PRO_EXECUTIVE
# (LINHA DE MONTAGEM, PERÍCIA TRI, RECOMPOSIÇÃO E EXPEDIÇÃO PARA IMPRESSÃO)
# ==============================================================================
elif menu == "📝 Central de Avaliações":
    st.title("Central de Avaliações")
    st.caption("Arquitetura de instrumentos avaliativos no padrão ENEM/SAEB/BNCC: ancoragem curricular, perícia psicométrica TRI, matriz inclusiva PEI e expedição para impressão.")
    st.markdown("---")
    ...
    with tab_forja:
        ...
        if f['fase'] == 1:
                ...
                with st.container(border=True):
                    ...
                    if "Acervo de Aulas SOSA" in fontes_ativas:
                        ...
                        if not df_ref.empty:
                            ...
                            mats_selecionados = c_safra1.multiselect("Aulas do Acervo:", options=df_ref["TIPO_MATERIAL"].tolist(), key=f"mats_sel_av_{v}")
                    ...
                    if "Livro Didático (PDF)" in fontes_ativas:
                        ...
                        if sel_livro_av:
                            ...
                            pags_teo_av = c_p_av1.text_input("Páginas de Teoria:", placeholder="Ex: 184-186", key=f"pags_teo_av_{v}")
                            pags_ex_av = c_p_av2.text_input("Páginas de Exercícios:", placeholder="Ex: 187-188", key=f"pags_ex_av_{v}")
                    ...
                    st.markdown("##### Prática de Sala de Aula (Questões Espelho)")
                    pincamento_pratica = st.text_area(
                        "Exercícios resolvidos em sala ou anotações da lousa:",
                        placeholder="Ex: Pág. 185: exercícios 2, 4 e 5. Exemplo do quadro sobre porcentagem no comércio local.",
                        height=75, key=f"pincamento_pratica_input_{v}"
                    )

                    if "Exercícios Complementares" in fontes_ativas:
                        recorte_provas_livro = st.text_area(
                            "Exercícios Autorais Adicionais:",
                            placeholder="Insira questões autorais ou trechos específicos...",
                    ...
                    if txt_av_teo_ext: contexto_base_texto += f"--- TEORIA DO LIVRO DIDÁTICO ---\n{txt_av_teo_ext}\n\n"
            ...
            st.markdown("### Fase 2: Forja & Lapidação de Itens (TRI)")
            st.caption("Revise, edite a perícia psicométrica e homologue cada item antes de avançar para as adaptações PEI.")
            ...
            with st.container(border=True):
                st.markdown("##### Distribuição Psicométrica dos Itens")
                col_tri1, col_tri2, col_tri3, col_tri4 = st.columns(4)
                col_tri1.metric("Total de Itens", total_q)
                col_tri2.metric("Itens Fáceis", f"{facil_c} ({perc_facil:.0f}%)")
                col_tri3.metric("Itens Médios", f"{media_c} ({perc_media:.0f}%)")
                col_tri4.metric("Itens Difíceis", f"{dificil_c} ({perc_dificil:.0f}%)")

            st.markdown("<br>", unsafe_allow_html=True)
            modo_leitura_forja = st.toggle("Renderizar Visualização Matemática", value=True, key=f"tog_read_forja_{v}")
            ...
            if pendentes:
                if st.button(f"Gerar Lote de {len(pendentes)} Itens com IA", type="primary", use_container_width=True, key=f"btn_lote_av_{v}"):
                    with st.spinner("Estruturando itens com ancoragem curricular..."):
                        prompt_lote = (
                            f"SÉRIE: {f['info']['ano']}\n"
                            f"VALOR TOTAL DA PROVA: {f['info']['valor']}\n\n"
                            f"DIRETRIZES DE ANCORAGEM:\n"
                            f"- Desenvolva cada questão focada estritamente no TEMA ESPECÍFICO atribuído.\n\n"
                        )
                        for item in pendentes:
                            prompt_lote += f"QUESTÃO {item['q']}:\n- TEMA: {item['tema']}\n- COMPLEXIDADE: {item['dificuldade']}\n- GABARITO: Letra {item['gabarito']}\n\n"
                        
                        prompt_lote += f"--- CONTEXTO E PRÁTICA CURRICULAR ---\n{f.get('contexto_base', 'Matriz curricular oficial do ' + str(f['info']['ano']) + ' Ano.')}\n"
            ...
            for i, item in enumerate(f['mapa']):
                ...
                assunto_exibicao = item['tema'] if item['tema'] and item['tema'].strip() not in ["**", ""] and not re.search(r"(?i)^(?:REVIS[AÃ]O|PROVA|TESTE|DOSSI[EÊ])", item['tema']) else f"Tópico Curricular - Item {item['q']:02d}"
                ...
                with st.container(border=True):
                    ...
                    if item['status'] == 'pendente':
                        ...
                        tema_q = c_t1.text_input("Objeto de Conhecimento do Item:", value=assunto_exibicao, key=f"t_{i}_{v}")
                        item['tema'] = tema_q
                        ...
                        if st.button(f"Gerar Item {item['q']} Individualmente", key=f"btn_gen_ind_{i}_{v}", use_container_width=True):
                        ...
                        if modo_leitura_forja:
                            with st.container(border=True):
                                st.markdown(f"**Conteúdo:** `{assunto_exibicao}`")
                                st.markdown(preparar_para_leitura(f"**QUESTÃO {item['q']:02d} -** {d['ENUNCIADO']}"))
                                
                                mark_a = "(Correta) " if d['GABARITO'] == 'A' else ""
                                mark_b = "(Correta) " if d['GABARITO'] == 'B' else ""
                                mark_c = "(Correta) " if d['GABARITO'] == 'C' else ""
                                mark_d = "(Correta) " if d['GABARITO'] == 'D' else ""
                                mark_e = "(Correta) " if d['GABARITO'] == 'E' else ""
                                ...
                                if d.get('ALT_E'): col_alt_a.markdown(f"**(E)** {mark_e}{preparar_para_leitura(d['ALT_E'])}")

                        with st.expander("Perícia TRI e Análise de Distratores", expanded=False):
                            st.caption(f"Descritor BNCC/SAEB: **{d.get('HABILIDADE', assunto_exibicao)}**")
                            st.write(f"Justificativa da Chave: {d.get('JUSTIFICATIVA', 'Não informada')}")
                            st.info(f"Análise de Distratores:\n{d.get('DISTRATORES', 'Não informada')}")

                        with st.expander("Editar Item Manualmente", expanded=False):
                            item['tema'] = st.text_input("Rótulo do Conteúdo:", value=assunto_exibicao, key=f"ed_tema_inp_{i}_{v}")
                            ...
                            d['JUSTIFICATIVA'] = st.text_area("Justificativa:", value=d.get('JUSTIFICATIVA', ''), height=50, key=f"ed_just_{i}_{v}")

                        inst_ref = st.text_input("Instrução de Refinamento com IA:", key=f"inst_ref_{i}_{v}")
                        ...
                        if col_b2.button(f"Regerar Item {item['q']}", key=f"btn_ref_{i}_{v}", use_container_width=True):
                            with st.spinner("Regerando item com IA..."):
            ...
            if todas_aprovadas and len(f['mapa']) > 0:
                st.markdown("<br>", unsafe_allow_html=True)
                st.success("Caderno Regular homologado com sucesso.")
                if st.button("Aprovar Caderno Regular e Avançar", type="primary", use_container_width=True, key=f"btn_fase2_next_{v}"):
                    f['fase'] = 3
                    st.rerun()

        elif f['fase'] == 3:
            st.markdown("### Fase 3: Matriz Inclusiva PEI (Sob Demanda)")
            st.caption("Selecione exclusivamente as adaptações necessárias para os estudantes laudados ou com necessidades específicas:")

            with st.container(border=True):
                niveis_selecionados = st.pills(
                    "Níveis de Acessibilidade Desejados:",
                    ["PEI Nível 1 (Apoio Leve - 3 Opções)", "PEI Nível 2 (Apoio Moderado - Guiado)", "PEI Nível 3 (Suporte Severo - No Papel)"],
                    default=["PEI Nível 1 (Apoio Leve - 3 Opções)", "PEI Nível 3 (Suporte Severo - No Papel)"],
                    selection_mode="multi",
            ...
            if st.button("Gerar Cadernos PEI", type="primary", use_container_width=True, key=f"btn_gen_triade_f3_{v}"):
                with st.status("Estruturando materiais adaptados...", expanded=True) as status:
                    ...
                    if pede_n1:
                        status.write("Estruturando PEI Nível 1 (3 Opções)...")
                    ...
                    status.update(label="Cadernos adaptados concluídos!", state="complete")
                    st.rerun()

            if f.get('pei_1') or f.get('pei_2') or f.get('pei_3'):
                modo_leitura_pei = st.toggle("Renderizar Visualização Matemática", value=True, key=f"read_pei_tog_{v}")
                ...
                if pede_n3 or f.get('pei_3'): abas_existentes_pei.append("PEI Nível 3 (No Papel)")
                ...
                if "PEI Nível 1" in abas_existentes_pei:
                    with tabs_pei[tab_idx]:
                        ...
                        f['pei_1'] = st.text_area("Edição PEI Nível 1:", value=f['pei_1'], height=220, key=f"ed_p1_area_{v}")
                    tab_idx += 1
                ...
                if "PEI Nível 2" in abas_existentes_pei:
                    with tabs_pei[tab_idx]:
                        ...
                        f['pei_2'] = st.text_area("Edição PEI Nível 2:", value=f['pei_2'], height=220, key=f"ed_p2_area_{v}")
                    tab_idx += 1

                if "PEI Nível 3 (No Papel)" in abas_existentes_pei:
                    with tabs_pei[tab_idx]:
                        if modo_leitura_pei:
                            with st.container(border=True):
                                st.markdown("##### Estrutura dos 10 Bento Boxes")
                                st.markdown(preparar_para_leitura(f['pei_3']))
                        f['pei_3'] = st.text_area("Edição PEI Nível 3:", value=f['pei_3'], height=220, key=f"ed_p3_area_{v}")

                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("Aprovar Adaptações e Avançar", type="primary", use_container_width=True, key=f"btn_fase3_next_{v}"):
                    f['fase'] = 4
                    st.rerun()

        elif f['fase'] == 4:
            st.markdown("### Fase 4: Custódia & Sincronização no Google Drive")
            st.caption("Compilação automática em arquivos DOCX oficiais com Cartão OMR Fiducial e links permanentes.")
            ...
            st.info("Acesse a aba 'Recomposição de Aprendizagem' para gerar cadernos de revisão ou a aba 'Expedição & Impressão' para gerar a remessa de xerox.")

            if st.button("Concluir e Retornar", use_container_width=True, key=f"btn_fin_f5_{v}"):
                reset_forja()
    ...
    with tab_acervo_av:
        ...
        with st.container(border=True):
            ...
            f_trim_sel = c_f_tri.segmented_control(
                "Trimestre:",
                ["Todos", "I Trimestre", "II Trimestre", "III Trimestre"],
                default="Todos",
                key=f"acervo_seg_trim_{v}"
            )
            
            f_ano_sel = c_f_ano.segmented_control(
                "Série:",
                ["Todas", "6º", "7º", "8º", "9º"],
                default="Todas",
                key=f"acervo_seg_ano_{v}"
            )
            
            f_tipo_pills = st.pills(
                "Categoria:",
                ["Todos", "Provas", "Testes", "Revisões", "2ª Chamada", "Sondas"],
                default="Todos",
                key=f"acervo_pills_tipo_{v}"
            )

        termos_avaliativos = [
            "AVALIAÇÃO", "AVALIACAO", "PROVA", "TESTE", "SONDA", 
            "RECUPERACAO", "RECUPERAÇÃO", "REVISAO", "REVISÃO", 
            "DIAGNÓSTICA", "DIAGNOSTICA"
        ]
        
        df_exames_todos = pd.DataFrame()
        if not df_aulas.empty and 'TIPO_MATERIAL' in df_aulas.columns:
            mask_todos_exames = (
                df_aulas['SEMANA_REF'].astype(str).str.upper().isin(["AVALIAÇÃO", "AVALIACAO", "REVISÃO", "REVISAO"]) |
                df_aulas['TIPO_MATERIAL'].astype(str).str.upper().str.contains('|'.join(termos_avaliativos), na=False)
            )
            df_exames_todos = df_aulas[mask_todos_exames].copy()

        df_exames = df_exames_todos.copy()

        if not df_exames.empty:
            if f_trim_sel != "Todos":
                padrao_regex_t = util.obter_regex_trimestre(f_trim_sel)
                mask_t = (
                    df_exames['TIPO_MATERIAL'].astype(str).str.contains(padrao_regex_t, regex=True, case=False, na=False) |
                    df_exames['CONTEUDO'].astype(str).str.contains(padrao_regex_t, regex=True, case=False, na=False) |
                    df_exames['SEMANA_REF'].astype(str).str.contains(padrao_regex_t, regex=True, case=False, na=False)
                )
                df_exames = df_exames[mask_t]
            ...
            if f_tipo_pills == "Provas":
                df_exames = df_exames[df_exames['TIPO_MATERIAL'].astype(str).str.upper().str.contains("PROVA") & ~df_exames['TIPO_MATERIAL'].astype(str).str.upper().str.contains("REVISAO|REVISÃO|2ª|2A|CHAMADA")]
            elif f_tipo_pills == "Testes":
                df_exames = df_exames[df_exames['TIPO_MATERIAL'].astype(str).str.upper().str.contains("TESTE") & ~df_exames['TIPO_MATERIAL'].astype(str).str.upper().str.contains("REVISAO|REVISÃO|2ª|2A|CHAMADA")]
            elif f_tipo_pills == "Revisões":
                df_exames = df_exames[df_exames['TIPO_MATERIAL'].astype(str).str.upper().str.contains("REVISAO|REVISÃO|RECOMPOSICAO|RECOMPOSIÇÃO")]
            elif f_tipo_pills == "2ª Chamada":
                df_exames = df_exames[df_exames['TIPO_MATERIAL'].astype(str).str.upper().str.contains("2ª|2A|CHAMADA")]
            elif f_tipo_pills == "Sondas":
                df_exames = df_exames[df_exames['TIPO_MATERIAL'].astype(str).str.upper().str.contains("SONDA|DIAGNÓSTICA|DIAGNOSTICA")]

            df_exames = df_exames.iloc[::-1]

        st.caption(f"Exibindo **{len(df_exames)} de {len(df_exames_todos)}** avaliações cadastradas.")
        ...
        if df_exames.empty:
            ...
            for idx_av, (_, row) in enumerate(df_exames.iterrows()):
                with st.container(border=True):
                    ...
                    if val_num == 0:
                        m_v = re.search(r"VALOR\s*[:\-]*\s*([\d\.,]+)", txt_f, re.IGNORECASE)
                        val_num = util.sosa_to_float(m_v.group(1)) if m_v else 4.0
                    val_ex = f"{val_num:.1f} pts"

                    trim_badge = "I Trimestre"
                    id_upper = identificador.upper()
                    txt_upper = txt_f.upper()
                    if any(x in id_upper or x in txt_upper for x in ["IITRIMESTRE", "II TRIMESTRE", "2º TRIMESTRE", "SEMANA 27", "SEMANA 26", "SEMANA 20"]):
                        trim_badge = "II Trimestre"
                    elif any(x in id_upper or x in txt_upper for x in ["IIITRIMESTRE", "III TRIMESTRE", "3º TRIMESTRE"]):
                        trim_badge = "III Trimestre"

                    cat_badge = "Prova Oficial"
                    if "TESTE" in id_upper: cat_badge = "Teste Oficial"
                    elif "REVISAO" in id_upper or "REVISÃO" in id_upper or "RECOMPOSICAO" in id_upper: cat_badge = "Caderno de Revisão"
                    elif "2ª" in id_upper or "2A" in id_upper or "CHAMADA" in id_upper: cat_badge = "Segunda Chamada"
                    elif "SONDA" in id_upper or "DIAGNÓSTICA" in id_upper: cat_badge = "Sonda Diagnóstica"
                    elif "RECUPERACAO" in id_upper or "RECUPERAÇÃO" in id_upper: cat_badge = "Recuperação Paralela"

                    c_card_h1, c_card_h2 = st.columns([3, 2])
                    c_card_h1.markdown(f"#### {identificador}")
                    c_card_h1.caption(f"Série: **{ano_exibicao}** | Data: **{data_exibicao}** | Valor: **{val_ex}** | Origem: *{semana_ref_exibicao}*")
                    c_card_h2.markdown(f"<div style='text-align: right;'><span style='background: #2962FF22; color: #2962FF; padding: 4px 10px; border-radius: 8px; font-weight: bold; font-size: 12px;'>{trim_badge}</span> <span style='background: #00E67622; color: #00E676; padding: 4px 10px; border-radius: 8px; font-weight: bold; font-size: 12px;'>{cat_badge}</span></div>", unsafe_allow_html=True)
                    ...
                    l_p2 = extrair_link_acervo_real(txt_f, "PEI_N2")
                    l_p3 = extrair_link_acervo_real(txt_f, "PEI_N3")
                    l_prof = extrair_link_acervo_real(txt_f, "Prof")

                    st.markdown("**Arquivos Oficiais no Google Drive:**")
                    cols_links = st.columns(5)
                    
                    tem_algum_link = False
                    col_idx = 0
                    if l_reg and "http" in str(l_reg):
                        cols_links[col_idx].link_button("Caderno Regular", str(l_reg), type="primary", use_container_width=True)
                        col_idx += 1; tem_algum_link = True
                    if l_p1 and "http" in str(l_p1):
                        cols_links[col_idx].link_button("PEI Nível 1", str(l_p1), use_container_width=True)
                        col_idx += 1; tem_algum_link = True
                    if l_p2 and "http" in str(l_p2):
                        cols_links[col_idx].link_button("PEI Nível 2", str(l_p2), use_container_width=True)
                        col_idx += 1; tem_algum_link = True
                    if l_p3 and "http" in str(l_p3):
                        cols_links[col_idx].link_button("PEI Nível 3", str(l_p3), use_container_width=True)
                        col_idx += 1; tem_algum_link = True
                    if l_prof and "http" in str(l_prof):
                        cols_links[col_idx].link_button("Guia Docente", str(l_prof), use_container_width=True)
                        col_idx += 1; tem_algum_link = True

                    if not tem_algum_link:
                        st.info("Documentos salvos no banco. Utilize a reexportação para gerar links diretos no Drive.")

                    st.markdown("<br>", unsafe_allow_html=True)

                    c_act1, c_act2, c_act3 = st.columns([2, 2, 1])

                    with c_act1.popover("Estruturar Plano no Ponto ID", use_container_width=True):
                        st.caption("A IA analisará as questões desta avaliação para redigir o plano de aula semanal correspondente.")
                        
                        todas_semanas_inj_a2 = util.gerar_semanas()
                        sem_dest_a2 = st.selectbox("Semana de Aplicação:", [s.split(" (")[0] for s in todas_semanas_inj_a2 if "Jornada" not in s], key=f"sel_sem_inj_a2_{row.name}_{idx_av}_{v}")
                        trim_dest_a2 = st.selectbox("Trimestre Alvo:", ["I Trimestre", "II Trimestre", "III Trimestre"], index=1 if "II" in trim_badge else 0, key=f"sel_trim_inj_a2_{row.name}_{idx_av}_{v}")
                        
                        if st.button("Confirmar e Gerar Plano", type="primary", use_container_width=True, key=f"btn_inj_acervo_{row.name}_{idx_av}_{v}"):
                            with st.spinner("Estruturando plano pedagógico..."):
                                prompt_plano_revisao = (
                                    f"SÉRIE: {ano_exibicao} Ano.\n"
                                    f"SEMANA: {sem_dest_a2}. TRIMESTRE: {trim_dest_a2}.\n"
                                    f"INSTRUMENTO: {identificador}.\n\n"
                                    f"SITUAÇÃO: O professor está aplicando/revisando este material em sala de aula. Analise o texto do material e extraia descritores SAEB reais e tópicos matemáticos para montar o Plano Semanal Padronizado.\n\n"
                                    f"TEXTO DO MATERIAL:\n{txt_f[:3500]}\n\n"
                                    f"USE AS TAGS OFICIAIS:\n"
                                    f"[HABILIDADE_BNCC] ...\n"
                                    f"[COMPETENCIAS_FOCO] Competência Específica 2 e 6 da BNCC\n"
                                    f"[OBJETO_CONHECIMENTO] {cat_badge.upper()} - {identificador}\n"
                                    f"[CONTEUDOS_ESPECIFICOS] ...\n"
                                    f"[OBJETIVOS_ENSINO] ...\n"
                                    f"[AULA_1] ...\n"
                                    f"[AULA_2] ...\n"
                                    f"[SABADO_LETIVO] N/A\n"
                                    f"[AVALIACAO_DE_MERITO] Aplicação e acompanhamento das resoluções.\n"
                                    f"[ESTRATEGIA_DUA_PEI] Adaptações PEI N1, N2 e N3 garantidas."
                                )
                                ...
                                st.session_state.p_meta = {
                                    ...
                                    "ano": ano_exibicao,
                                    "base": f"Material: {identificador}",
                                    "status_final": "PRODUZIDO"
                                }
                                
                                st.toast("Plano estruturado no Ponto ID!", icon="✅")
                                st.cache_data.clear()
                                time.sleep(0.6)
                                navegar_para("📅 Planejamento (Ponto ID)")

                    with c_act2.popover("Reexportar Documento no Drive", use_container_width=True):
                        st.caption("Recompila os arquivos oficiais DOCX no Google Drive a partir do texto preservado no banco.")
                        
                        if st.button("Executar Reexportação", type="primary", use_container_width=True, key=f"btn_reexp_{row.name}_{idx_av}_{v}"):
                            with st.status("Recompilando documentos DOCX...", expanded=True) as status_reexp:
                                info_reexp = {
                                    'ano': ano_exibicao, 
                                    'trimestre': f_trim_sel if f_trim_sel != "Todos" else ('II Trimestre' if "II" in trim_badge else 'I Trimestre'), 
                                    'valor': val_ex if val_ex else '4.0',
                                    'qtd': len(re.findall(r'(?i)QUESTÃO\s+\d+', txt_f)) or 10
                                }
                                
                                status_reexp.write("Recompilando Caderno Regular...")
                                ...
                                link_p1_r = "N/A"
                                if pei1_txt_r:
                                    status_reexp.write("Recompilando PEI Nível 1...")
                                ...
                                link_p3_r = "N/A"
                                if pei3_txt_r:
                                    status_reexp.write("Recompilando PEI Nível 3...")
                                ...
                                status_reexp.update(label="Documentos reexportados com sucesso!", state="complete")
                                st.balloons(); time.sleep(0.8); st.rerun()

                    if c_act3.button("Excluir", key=f"del_ac_{row.name}_{idx_av}_{v}", use_container_width=True):
                        if db.excluir_avaliacao_completa(identificador, semana_ref_exibicao):
                            st.success(f"{identificador} excluído com sucesso.")
                            time.sleep(0.5); st.rerun()

                    with st.expander("Perícia Psicométrica TRI & Distratores", expanded=False):
                        t_pericia, t_caderno, t_inclusao = st.tabs(["Perícia de Distratores", "Caderno Regular", "Adaptação PEI"])
                                ...
                                if len(questoes_grade) > 1:
                                    for idx_g in range(1, len(questoes_grade), 2):
                                        ...
                                        with st.container(border=True):
                                            ...
                                            if m_hab: st.caption(f"Descritor: **{m_hab.group(1).strip()}**")
                                            if m_just: st.write(f"Chave Correta: {m_just.group(1).strip()}")
                                            if m_peri: st.info(f"Análise de Distratores: {m_peri.group(1).strip()}")
                                else: st.text(grade_raw)
                            else: st.info("Perícia psicométrica não anexada a este registro.")

                        with t_caderno:
                            questoes_reg = ai.extrair_tag(txt_f, "QUESTOES")
                            if questoes_reg: st.write(preparar_para_leitura(questoes_reg))
                            else: st.info("Enunciados preservados nos arquivos oficiais.")

                        with t_inclusao:
                            pei_txt = ai.extrair_tag(txt_f, "PEI_NIVEL_1") or ai.extrair_tag(txt_f, "NIVEL_1") or ai.extrair_tag(txt_f, "PEI")
                            if pei_txt: st.write(preparar_para_leitura(pei_txt))
                            else: st.info("Adaptações inclusivas disponíveis no arquivo PEI do Drive.")
    ...
    with tab_recomposicao:
        ...
        if df_provas_para_revisao.empty:
            ...
            if prova_sel_recomposicao:
                ...
                with st.container(border=True):
                    st.markdown("#### Parâmetros do Caderno de Recomposição")
                    c_aut_r1, c_aut_r2 = st.columns([1, 1])
                    
                    qtd_q_recomp = c_aut_r1.segmented_control(
                        "Quantidade de Questões:",
                        ["3 Questões", "5 Questões", "10 Questões"],
                        default="5 Questões",
                        key=f"pills_qtd_recomp_{v}"
                    )
                    
                    estrat_recomp = c_aut_r2.segmented_control(
                        "Estratégia Pedagógica:",
                        ["Questões Espelho SAEB", "Análise Guiada de Erros", "Estudo Dirigido"],
                        default="Questões Espelho SAEB",
                ...
                if st.button("Gerar Caderno de Recomposição", type="primary", use_container_width=True, key=f"btn_exe_recomp_{v}"):
                    with st.status("Estruturando material de recomposição completo...", expanded=True) as status_rec:
                        num_q_num = int(re.search(r'\d+', str(qtd_q_recomp)).group(0)) if re.search(r'\d+', str(qtd_q_recomp)) else 5
                        ...
                        status_rec.write("1/4 Estruturando Guia Docente e Exercícios...")
                        prompt_recomposicao = (
                            f"PROVA BASE DE ORIGEM:\n{txt_prova_orig}\n\n"
                            f"SÉRIE: {ano_rev_sel}.\n"
                            f"QUANTIDADE DE QUESTÕES: {num_q_num} questões.\n"
                            f"ESTRATÉGIA: {estrat_recomp}.\n\n"
                            f"MISSÃO: Crie o Caderno de Recomposição com Roteiro do Professor ([PROFESSOR]), Exercícios para os Alunos ([ALUNO]), Gabarito ([GABARITO_TEXTO]), Perícia TRI ([GRADE_DE_CORRECAO]), PEI N1 ([PEI_NIVEL_1]), PEI N2 ([PEI_NIVEL_2]) e PEI N3 em 10 Bento Boxes ([PEI_NIVEL_3])."
                        )
                        res_recomposicao = ai.gerar_ia("ARQUITETO_REVISAO_V29", prompt_recomposicao)
                        
                        txt_prof = ai.extrair_tag(res_recomposicao, "PROFESSOR") or ai.extrair_tag(res_recomposicao, "ROTEIRO_DO_PROFESSOR") or "Guia Docente."
                        txt_alu = ai.extrair_tag(res_recomposicao, "ALUNO") or ai.extrair_tag(res_recomposicao, "QUESTOES") or "Exercícios."
                        ...
                        status_rec.write("2/4 Compilando Caderno Regular DOCX...")
                        ...
                        status_rec.write("3/4 Compilando Cadernos Inclusivos PEI...")
                        ...
                        status_rec.write("4/4 Compilando Guia Docente DOCX...")
                ...
                if f"recomp_gerada_{v}" in st.session_state:
                    ...
                    st.success(f"Caderno **{rec_data['nome']}** disponível no acervo.")
                    
                    c_link1, c_link2 = st.columns(2)
                    c_link1.link_button("Abrir Caderno no Google Drive", rec_data['link_alu'], type="primary", use_container_width=True)
                    
                    with c_link2.popover("Estruturar Plano no Ponto ID"):
                        st.caption("Transfira este material como plano semanal no Ponto ID.")
                        
                        todas_semanas_inj = util.gerar_semanas()
                        sem_destino_inj = st.selectbox("Semana de Aplicação:", [s.split(" (")[0] for s in todas_semanas_inj if "Jornada" not in s], key=f"sel_sem_inj_{v}")
                        trim_destino_inj = st.selectbox("Trimestre:", ["I Trimestre", "II Trimestre", "III Trimestre"], index=["I Trimestre", "II Trimestre", "III Trimestre"].index(rec_data['trimestre']) if rec_data['trimestre'] in ["I Trimestre", "II Trimestre", "III Trimestre"] else 0, key=f"sel_trim_inj_{v}")
                        
                        if st.button("Confirmar e Enviar para Ponto ID", type="primary", use_container_width=True, key=f"btn_conf_inj_{v}"):
                            roteiro_a1 = f"AULA 01 - DEVOLUTIVA PEDAGÓGICA E RECOMPOSIÇÃO:\nINÍCIO: Análise dos descritores críticos e devolutiva formativa.\nMEIO: Resolução comentada no quadro do Caderno de Recomposição ({rec_data['nome']}).\nFIM: Síntese e esclarecimento de dúvidas."
                            roteiro_a2 = f"AULA 02 - FIXAÇÃO DE DESCRITORES CRÍTICOS:\nINÍCIO: Orientação para resolução individual guiada.\nMEIO: Mediação individualizada aos estudantes com suporte PEI.\nFIM: Verificação de vistos nos cadernos."
                            ...
                            plano_texto_rascunho = (
                                ...
                                f"[CONTEUDOS_ESPECIFICOS] Consolidação dos conteúdos com menor índice de acerto na avaliação.\n"
                                f"[OBJETIVOS_ENSINO] Superar lacunas cognitivas identificadas nas avaliações do {rec_data['ano']}.\n"
                                f"[JUSTIFICATIVA_PEDAGOGICA] Recomposição contínua de aprendizagem orientada por dados.\n"
                                f"[AULA_1] {roteiro_a1}\n"
                                f"[AULA_2] {roteiro_a2}\n"
                                f"[SABADO_LETIVO] N/A\n"
                                f"[AVALIACAO_DE_MERITO] Acompanhamento do engajamento e visto na resolução do caderno.\n"
                                f"[ESTRATEGIA_DUA_PEI] Cadernos adaptados (PEI N1, N2 e N3) com suporte visual e mediação."
                            )
                            ...
                            st.toast("Caderno transferido para o Ponto ID!", icon="✅")
                            time.sleep(0.6)
                            navegar_para("📅 Planejamento (Ponto ID)")

    # ==============================================================================
    # ABA 4: EXPEDIÇÃO & IMPRESSÃO
    # ==============================================================================
    with tab_expedicao:
        @st.fragment
        def renderizar_expedicao_fragmento():
            st.markdown("### Expedição & Impressão")
            st.caption("Gerador da remessa oficial para impressão com contagem exata de cópias por turma e alunos PEI nominalmente identificados.")

            c_exp1, c_exp2 = st.columns([1, 2])
            trim_exp_sel = c_exp1.selectbox("Trimestre:", ["I Trimestre", "II Trimestre", "III Trimestre"], key=f"exp_trim_sel_{v}")

            turmas_disp_exp = sorted(df_alunos['TURMA'].unique().tolist()) if not df_alunos.empty else []
            turmas_sel_exp = c_exp2.multiselect("Turmas para Remessa:", options=turmas_disp_exp, default=turmas_disp_exp[:min(3, len(turmas_disp_exp))], key=f"exp_turmas_sel_{v}")

            if not turmas_sel_exp:
                st.info("Selecione ao menos uma turma para compor a remessa de impressão.")
            else:
                ...
                nome_exame_email = st.selectbox("Instrumento Avaliativo:", options=["AVALIAÇÃO TRIMESTRAL DE MATEMÁTICA"] + opcoes_provas_exp, key=f"exp_exame_nome_{v}")
                ...
                for t_item in turmas_sel_exp:
                    ...
                    bloco_t += f"• Cadernos Regulares: {reg_count:02d} cópias\n"
                    if p1_names:
                        bloco_t += f"• Caderno PEI Nível 1: {len(p1_names):02d} cópia(s) (Estudante(s): {', '.join(p1_names)})\n"
                    if p2_names:
                        bloco_t += f"• Caderno PEI Nível 2: {len(p2_names):02d} cópia(s) (Estudante(s): {', '.join(p2_names)})\n"
                    if p3_names:
                        bloco_t += f"• Caderno PEI Nível 3: {len(p3_names):02d} cópia(s) (Estudante(s): {', '.join(p3_names)})\n"

                    linhas_email_turmas.append(bloco_t)

                tot_geral_copias = tot_reg_geral + tot_p1_geral + tot_p2_geral + tot_p3_geral

                corpo_email_oficial = f"""Prezados(as) da Direção, Coordenação e Setor de Impressão,

Encaminho o detalhamento de impressões da avaliação referente ao {trim_exp_sel} ({nome_exame_email}):

""" + "\n".join(linhas_email_turmas) + f"""
==================================================
RESUMO CONSOLIDADO DE CÓPIAS:
• Cadernos Regulares: {tot_reg_geral:02d}
• Cadernos PEI Nível 1: {tot_p1_geral:02d}
• Cadernos PEI Nível 2: {tot_p2_geral:02d}
• Cadernos PEI Nível 3: {tot_p3_geral:02d}
• TOTAL GERAL DE CÓPIAS: {tot_geral_copias:02d} cópias
...
"""
                ...
                st.markdown("#### Documento de Remessa Oficial")
                st.code(corpo_email_oficial, language=None)

                with st.container(border=True):
                    st.markdown("##### Resumo Quantitativo de Impressão")
                    ...
                    c_k5.metric("Total Cópias", tot_geral_copias)

                st.markdown("---")
                st.markdown(f"#### Acervo Trimestral de Segurança ({trim_exp_sel})")
                st.caption("Documentos congelados no Drive para impressão e custódia permanente.")
                ...
                if df_provas_trim_acervo.empty:
                    st.info(f"Nenhuma avaliação arquivada para o {trim_exp_sel}.")
                else:
                    for idx_exp, (_, row_ac) in enumerate(df_provas_trim_acervo.iterrows()):
                        with st.container(border=True):
                            ...
                            if l_docx and "http" in str(l_docx):
                                c_a2.link_button("Abrir DOCX", str(l_docx), use_container_width=True)
                            else:
                                c_a2.caption("DOCX Indisponível")

                            if c_a3.button("Gerar PDF no Drive", key=f"btn_pdf_freeze_{row_ac.name}_{idx_exp}_{v}", use_container_width=True):
                                with st.spinner("Compilando PDF no Google Drive..."):
                                    ...
                                    st.success("PDF gerado no Drive com sucesso!")
...
# ==============================================================================
    ...
    st.caption("Leitura óptica de gabaritos por visão computacional direta, auditoria psicométrica TRI, validação de titularidade e tribunal de recursos.")
    st.markdown("---")
    ...
    # ==============================================================================
    # ABA 1: MESA DE CORREÇÃO (COM DETECTOR ANTI-TROCA DE NOME)
    # ==============================================================================
    with tab_correcao:
                    ...
                    if alunos_alvo:
                        ...
                        def renderizar_mesa_correcao_fragmento():
                            ...
                            if material_ref is not None:
                                ...
                                if is_av_discursiva:
                                    st.info(f"Mesa de Lançamento Discursivo: Instrumento aberto valendo de 0,0 a {v_total_at:.1f} pontos.")
                                    
                                    c_disc1, c_disc2 = st.columns([1.2, 1.8])
                                    nota_discursiva_inp = c_disc1.number_input(
                                        f"Pontuação Obtida (0.0 a {v_total_at:.1f}):",
                                        min_value=0.0, max_value=float(v_total_at), value=float(min(v_total_at, 6.0)), step=0.5,
                                        key=f"inp_nota_disc_{v}"
                                    )
                                    
                                    img_disc_file = c_disc2.file_uploader("Anexar Imagem da Avaliação (Opcional):", type=["jpg", "jpeg", "png"], key=f"up_disc_ev_{v}")

                                    c_b_disc1, c_b_disc2 = st.columns(2)
                                    if c_b_disc1.button("Consolidar Nota Discursiva", type="primary", use_container_width=True, key=f"btn_save_disc_{v}"):
                                        with st.spinner("Registrando pontuação discursiva..."):
                                            ...
                                            st.success(f"Nota de {alunos_alvo[0]} ({nota_discursiva_inp:.1f} pts) homologada com sucesso!")
                                            time.sleep(0.6); st.rerun()

                                    if c_b_disc2.button("Registrar Ausência", use_container_width=True, key=f"btn_aus_disc_{v}"):
                                        for aluno_nome in alunos_alvo:
                                            ...
                                            if erros_detalhados_tri:
                                                with st.expander("Diagnóstico de Distratores dos Itens Incorretos", expanded=False):
                                                    for err_txt in erros_detalhados_tri:
                                                        st.info(preparar_para_leitura(err_txt))
            ...
            if not t_sel_r or not at_sel_r:
                ...
                if respostas_brutas.empty:
                    ...
                    def classificar_caderno(row):
                        ...
                        if "2ª" in id_av or "2CHAMADA" in id_av: return "Segunda Chamada (Discursiva)"
                    ...
                    if not cadernos_disponiveis:
                        ...
                        is_2a_chamada = "Segunda Chamada" in str(caderno_alvo)
                        ...
                        if is_2a_chamada:
                            ...
                            for i in range(1, num_q_total + 1):
                                ...
                                acertos_parciais = votos.count("Acerto Parcial")
...
# ==============================================================================
# MÓDULO: PAINEL DE NOTAS & VISTOS - V2026.LIVE_DYNAMIC (SOSA SOBERANO)
# (SINCRONIZAÇÃO INSTANTÂNEA DO SCANNER CIR, VISTOS AO VIVO E ARREDONDAMENTO 0.5)
# ==============================================================================
elif menu == "📊 Painel de Notas & Vistos":
    st.title("📊 Painel de Notas, Vistos & Ciclos")
    st.caption("Central de Soberania Docente: Sincronização Dinâmica ao Vivo do Scanner CIR, Consolidação C1/C2/C3, Gestão Atitudinal e Recuperação Blindada.")
    st.markdown("---")
    ...
    if not lista_turmas_notas:
        ...
        # TRAVA GLOBAL: APENAS ALUNOS ATIVOS NO PAINEL DE NOTAS (LEI #28)
        ...
        if alunos_notas_df.empty:
            ...
            def renderizar_painel_notas_fragmento():
                # -------------------------------------------------------------
                # 1. GESTÃO DE DATAS DE CORTE DO TRIMESTRE
                # -------------------------------------------------------------
                ...
                # -------------------------------------------------------------
                # 2. MOTOR DINÂMICO AO VIVO: CAPTURA DO SCANNER CIR EM TEMPO REAL
                # -------------------------------------------------------------
                ...
                # Mineração Instantânea de DB_GABARITOS_ALUNOS (Scanner CIR)
                mapa_live_teste = {}
                mapa_live_prova = {}
                mapa_live_rec = {}
                ...
                # Mineração Instantânea de Vistos e Bônus de DB_DIARIO_BORDO
                vistos_live_dict = {}
                bonus_live_dict = {}
                ...
                for _, al_n in alunos_notas_df.iterrows():
                    ...
                    # Fusão Inteligente: Dá prioridade ao escaneamento ao vivo do Scanner CIR
                    ...
                    if media_arredondada >= 6.0:
                        ...
                        convocados_recuperacao.append({
                            ...
                            "Média Atual": media_arredondada, "Precisa na Rec": round((6.0 - media_arredondada) * 2) / 2
                        })
                    ...
                    dados_grid_notas.append({
                        ...
                        "REC": rec_val if rec_val > 0 else 0.0,
                ...
                # -------------------------------------------------------------
                # 4. NAVEGAÇÃO EM 3 VISÕES EXECUTIVAS LIMPAS
                # -------------------------------------------------------------
                visao_selecionada = st.segmented_control(
                    "Selecione a Visão:",
                    ["📋 1. Consolidador de Notas", "⚖️ 2. Vistos & Atitude (Controle de Sala)", "📑 3. Recuperação & Refacção"],
                    default="📋 1. Consolidador de Notas",
                    key=f"seg_visao_notas_{v}"
                )
                st.markdown("<br>", unsafe_allow_html=True)

                # ==============================================================
                # VISÃO 1: CONSOLIDADOR DE NOTAS & BOLETIM
                # ==============================================================
                if visao_selecionada == "📋 1. Consolidador de Notas":
                    with st.container(border=True):
                        c_head_c1, c_head_c2 = st.columns([3, 1])
                        c_head_c1.caption(f"📅 **Período Ativo de Vistos ({trim_ativo_notas}):** {dt_i_n.strftime('%d/%m/%Y')} até {dt_f_n.strftime('%d/%m/%Y')} • *Notas escaneadas sincronizadas dinamicamente ao vivo.*")
                        
                        with c_head_c2.popover("⚙️ Configurar Período / Cadeado", use_container_width=True):
                            st.markdown("#### 🔒 Cadeado de Vistos & Data de Corte")
                            st.caption("Ajuste a data de corte para fechar o trimestre e começar o próximo antecipadamente.")
                            nova_dt_corte = st.date_input("Data de Corte dos Vistos:", dt_f_n, format="DD/MM/YYYY", key=f"inp_corte_pop_{v}")
                    ...
                    df_notas_editado = st.data_editor(
                        ...
                        column_config={
                            "ID": None,
                            "Estudante": st.column_config.TextColumn("Estudante", disabled=True, width="medium"),
                            "C1 (Vistos - Teto 3.0)": st.column_config.NumberColumn("C1 (Vistos)", format="%.1f", min_value=0.0, max_value=3.0, width="small"),
                            "C2 (Testes - Teto 3.0)": st.column_config.NumberColumn("C2 (Testes)", format="%.1f", min_value=0.0, max_value=3.0, width="small"),
                            "C3 (Prova - Teto 4.0)": st.column_config.NumberColumn("C3 (Prova)", format="%.1f", min_value=0.0, max_value=4.0, width="small"),
                            "⭐ Bônus/Punição Líquido": st.column_config.NumberColumn("⭐ Bônus Líquido", format="%.1f", disabled=True, width="small"),
                            "REC": st.column_config.NumberColumn("REC", format="%.1f", min_value=0.0, max_value=10.0, width="small"),
                            "Média Final": st.column_config.NumberColumn("Média Final", format="%.1f", disabled=True, width="small"),
                ...
                # ==============================================================
                    ...
                    with col_rec_main:
                        with st.container(border=True):
                            st.markdown("#### 📑 Recuperação Paralela Blindada")
                            ...
                            if not convocados_recuperacao:
                                st.success(f"🏆 Todos os alunos da turma {turma_notas} estão na média!")
                            else:
                                df_conv_rec = pd.DataFrame(convocados_recuperacao)
                                st.dataframe(
                                    df_conv_rec[['Estudante', 'Média Atual', 'Precisa na Rec']],
                                    hide_index=True, use_container_width=True, height=220,
                                    column_config={
                                        "Estudante": st.column_config.TextColumn(width="medium"),
                                        "Média Atual": st.column_config.NumberColumn(format="%.1f"),
                                        "Precisa na Rec": st.column_config.NumberColumn("🎯 Precisa", format="%.1f")
                                    }
                                )

                                if st.button("🖨️ GERAR LISTA DE RECUPERAÇÃO (DOCX)", type="primary", use_container_width=True, key=f"btn_docx_conv_clean_{v}"):
                    ...
                    with col_ref_main:
                        with st.container(border=True):
                            ...
                            st.caption("Atribua +0.5 para alunos que refizerem atividades ou provas para alcançar a média 6.0.")
                            
                            aluno_ref_sel = st.selectbox("Estudante:", [r['Estudante'] for _, r in df_grid_ed_notas.iterrows()], key=f"sel_al_ref_clean_{v}")
                            c_r1, c_r2 = st.columns(2)
                            pts_refaccao = c_r1.number_input("Acréscimo (+ pts):", 0.0, 2.0, 0.5, step=0.5, key=f"inp_pts_ref_clean_{v}")
                            alvo_refaccao = c_r2.selectbox("Aplicar em:", ["⭐ Bônus de Caderno", "C2 (Testes)", "C3 (Prova)"], key=f"sel_alvo_ref_clean_{v}")

                            if aluno_ref_sel:
                                row_ref_al = next(r for _, r in df_grid_ed_notas.iterrows() if r['Estudante'] == aluno_ref_sel)
                                id_al_ref = row_ref_al['ID']
                                media_atual_ref = row_ref_al['Média Final']
                                c1_ref = row_ref_al['C1 (Vistos - Teto 3.0)']
                                ...
                                c_m_sim2.metric("Nova Média", f"{nova_m_simulada:.1f}", delta=f"+{nova_m_simulada - media_atual_ref:.1f} pts")

                                if nova_m_simulada >= 6.0:
                                    st.success("🎉 **ESTUDANTE ATINGIU A MÉDIA 6.0!**")

                                if st.button("💾 GRAVAR REFACÇÃO NO BANCO", type="primary", use_container_width=True, key=f"btn_save_ref_al_clean_{v}"):
                                    with st.spinner("Gravando refacção no banco..."):
                                        ...
                                        db.limpar_notas_turma_trimestre(turma_notas, trim_ativo_notas)
                                        st.cache_data.clear()
                                        st.success("✅ Refacção gravada!"); time.sleep(0.8); st.rerun()
...
# ==============================================================================
# MÓDULO: GESTÃO DA TURMA (COCKPIT DE REGÊNCIA 360°) - V2026.PRO_COCKPIT
# (TIMELINE CRONOLÓGICA ÚNICA, PROFICIÊNCIA SAEB, RADAR DUPLO & GESTÃO COMPLETA)
# ==============================================================================
elif menu == "👥 Gestão da Turma":
    st.title("👥 Cockpit de Regência: Gestão 360°")
    st.caption("Central de comando operacional: diário de classe dinâmico, chamada e vistos em 1-toque, radiografia analítica SAEB e gestão administrativa.")
    st.markdown("---")
    ...
    def dialog_roleta(t_roleta):
        ...
        # Filtro de alunos ativos da turma
        ...
        if alunos_roleta.empty: 
            st.warning("Nenhum aluno ativo cadastrado nesta turma.")
            return
        ...
        if c_btn_sort.button("🎲 SORTEAR ESQUADRÃO (PRIORIZANDO MENOS CHAMADOS)", type="primary", use_container_width=True, key=f"btn_sort_{v}"):
            if not pendentes: st.success("Todos já foram chamados nesta aula!")
            else: 
        ...
        if c_btn_reset.button("🔄 Resetar Lista de Sorteio", use_container_width=True, key=f"btn_res_rol_{v}"):
        ...
        if st.session_state[chave_sorteados]:
            ...
            for idx, id_atual in enumerate(st.session_state[chave_sorteados]):
                with cols[idx]:
                    ...
                    with st.container(border=True):
                        st.markdown(f"<h5 style='text-align: center; margin-bottom: 0px;'>{aluno_atual.get('Estudante', 'Estudante')}</h5>", unsafe_allow_html=True)
                        st.caption(f"Perfil: {aluno_db.get('NECESSIDADES', 'TÍPICO')} | Chamadas no Tri: **{aluno_atual.get('Chamadas', 0)}**")
                        anotacao = st.text_area("📝 Diagnóstico:", value=aluno_atual.get("Diagnóstico", ""), key=f"anotacao_{id_atual}_{v}", height=68)
    ...
    @st.dialog("🛑 Registrar Dia Não Letivo / Paralisação")
    def dialog_dia_nao_letivo(t_foco):
        st.caption("Registre feriados, paralisações sindicais ou eventos institucionais. Bloqueia chamadas de falta indevidas.")
        data_nl = st.date_input("Data do Evento:", date.today(), format="DD/MM/YYYY", key=f"nl_dt_{v}")
        motivo_nl = st.text_input("Motivo da Inatividade:", placeholder="Ex: Paralisação Sindical / Conselho de Classe / Feriado Municipal", key=f"nl_mot_{v}")
        ...
        if st.button("Confirmar Dia Não Letivo", type="primary", use_container_width=True, key=f"btn_conf_nl_{v}"):
            if motivo_nl:
                ...
                st.success(f"✅ Dia {data_nl_str} registrado como Não Letivo!"); time.sleep(1); st.rerun()
            else: st.error("Digite o motivo do dia não letivo.")

    if not lista_turmas_segura:
        st.warning("⚠️ Nenhuma turma cadastrada. Cadastre as turmas na Central Administrativa abaixo.")
    else:
        hoje_dt = date.today()
        ...
        # CABEÇALHO EXECUTIVO 360° DA TURMA
        with st.container(border=True):
            c_head1, c_head2 = st.columns([1.5, 2])
            turma_foco = c_head1.selectbox("🎯 Turma Selecionada:", lista_turmas_segura, key=f"foco_t_{v}")
            ...
            st.caption(f"📊 **Cumprimento de Safra ({trim_ativo_gestao}):** {aulas_dadas_trim} de {aulas_meta_trim} Aulas Ministradas ({perc_safra:.0f}%)")

        # Trava Global: Apenas alunos ATIVOS aparecem nos fluxos diários da turma (Lei #28)
        ...
        # BARRA DE ALERTAS CRÍTICOS NO TOPO DO COCKPIT
        ...
        if cnt_uti > 0 or cnt_evasao > 0 or cnt_atestados > 0:
            with st.container(border=True):
                st.markdown("##### 🚨 Radar de Risco & Alertas Imediatos")
                c_al1, c_al2, c_al3 = st.columns(3)
                
                if cnt_evasao > 0: c_al1.error(f"🚨 **{cnt_evasao} Estudante(s)** em Risco de Evasão (≥ 20% faltas)")
                else: c_al1.success("✅ Nenhum aluno em risco de evasão")

                if cnt_uti > 0: c_al2.warning(f"🚑 **{cnt_uti} Estudante(s)** na UTI Pedagógica (< 6.0)")
        ...
        # NAVEGAÇÃO EM 3 CENTRAIS DE COMANDO ESTRATÉGICAS (ZERO SCROLL INFINITO)
        visao_gestao_sel = st.segmented_control(
            "Central de Gestão:",
            ["🚀 1. Diário de Classe & Operação", "🧠 2. Radiografia Analítica 360°", "⚙️ 3. Secretaria & Calendário"],
            default="🚀 1. Diário de Classe & Operação",
            key=f"seg_gestao_hub_{v}"
        )
        st.markdown("<br>", unsafe_allow_html=True)

        # ==============================================================================
        # FRAGMENTO DE ALTA PERFORMANCE (LEI #21 - ZERO LAG)
        # ==============================================================================
        @st.fragment
        def renderizar_cockpit_gestao_fragmento():
            
            # ==============================================================
            # VISÃO 1: DIÁRIO DE CLASSE DINÂMICO & OPERAÇÃO DE SALA
            # ==============================================================
            if visao_gestao_sel == "🚀 1. Diário de Classe & Operação":
                
                # Barra Superior de Ações Rápidas da Sala
                c_top_act1, c_top_act2, c_top_act3 = st.columns([2, 1, 1])
                c_top_act1.markdown("#### 🚀 Diário de Classe & Timeline")
                ...
                # -------------------------------------------------------------
                # 1. TIMELINE CRONOLÓGICA UNIFICADA (HOJE / ÚLTIMA / QUALQUER DATA)
                # -------------------------------------------------------------
                ...
                with st.container(border=True):
                    ...
                    modo_data_timeline = c_time1.pills(
                        "Selecione a Aula:",
                        [f"🟢 Aula de Hoje ({hoje_str})", f"⏪ Última Aula ({ultima_aula_str})", "📅 Escolher Outra Data"],
                    ...
                    if "Hoje" in modo_data_timeline:
                        ...
                        dt_pick = c_time2.date_input("Escolha a Data do Diário:", date.today(), format="DD/MM/YYYY", key=f"dt_pick_timeline_{v}")
                        data_ativa_str = dt_pick.strftime("%d/%m/%Y")

                    # DNA da Aula Selecionada
                    ...
                    if aula_aberta_existe:
                        ...
                        st.info(f"💡 **Aula do dia {data_ativa_str} ainda não foi aberta.** Você pode abrir abaixo ou lançar presença diretamente.")

                # -------------------------------------------------------------
                # 2. ABERTURA RÁPIDA DE AULA (SE NECESSÁRIO)
                # -------------------------------------------------------------
                if not aula_aberta_existe:
                    with st.expander("📦 Abrir Aula e Vincular Material do Acervo", expanded=False):
                        mats_disp_bruto = df_mats_ano['TIPO_MATERIAL'].tolist() if not df_mats_ano.empty and 'TIPO_MATERIAL' in df_mats_ano.columns else []
                        mats_sel_abertura = st.multiselect("Selecione o Material / Aula do Acervo:", options=mats_disp_bruto, max_selections=2, key=f"ms_mat_ab_{v}")
                        
                        if st.button("💾 CONFIRMAR ABERTURA DESTA AULA", type="primary", use_container_width=True, key=f"btn_abrir_aula_tl_{v}"):
                ...
                # -------------------------------------------------------------
                # 3. CHAMADA EXPRESS & VISTOS DA DATA ATIVA
                # -------------------------------------------------------------
                st.markdown(f"##### 📋 Chamada & Vistos — Aula de **{data_ativa_str}**")
                ...
                for _, al_c in alunos_t.iterrows():
                    ...
                    grid_chamada_dia.append({
                        ...
                        "Presente?": is_p_c,
                        "Visto OK?": is_v_c,
                        "Ocorrência / Tag": tag_c if tag_c != "AUSÊNCIA" else "",
                        "Observação": obs_c
                    })

                # Botões Rápidos de Lote
                c_lot1, c_lot2, c_lot3 = st.columns([1, 1, 2])
                ...
                df_chamada_editada = st.data_editor(
                    ...
                    column_config={
                        ...
                        "Presente?": st.column_config.CheckboxColumn("Presente?", default=True, width="small"),
                        "Visto OK?": st.column_config.CheckboxColumn("Visto OK?", default=False, width="small"),
                        "Ocorrência / Tag": st.column_config.SelectboxColumn("Ocorrência", options=["", "ARGUIÇÃO", "INDISCIPLINA", "CELULAR", "CONVERSA", "ATRASO", "DESTAQUE"], width="medium"),
                ...
                if c_save_ch1.button("💾 CONSOLIDAR E SALVAR DIÁRIO DESTA AULA", type="primary", use_container_width=True, key=f"btn_save_diario_tl_{v}"):
                    with st.spinner("Gravando presenças, vistos e ocorrências no diário..."):
                        ...
                        for _, r_c in df_chamada_editada.iterrows():
                            ...
                            p_c = r_c['Presente?']
                            v_c = r_c['Visto OK?']
                            t_c = str(r_c['Ocorrência / Tag']).strip()
                        ...
                        if linhas_diario_save:
                            ...
                            st.toast(f"✅ Diário do dia {data_ativa_str} consolidado com sucesso!", icon="✅")
                            time.sleep(0.5); st.rerun()

                # -------------------------------------------------------------
                # 4. RADAR MÓVEL DE FALTOSOS (PARA A SECRETARIA / WHATSAPP)
                # -------------------------------------------------------------
                st.markdown("---")
                qtd_ausentes_hoje = len(ids_ausentes_dia)
                total_alunos_classe = len(alunos_t)

                with st.container(border=True):
                    c_rad1, c_rad2 = st.columns([2, 1])
                    c_rad1.markdown(f"##### 🔴 Radar de Faltosos — Aula de {data_ativa_str} ({qtd_ausentes_hoje} ausentes)")
                    c_rad1.caption("Toque em **'Dar Presença'** para retificar qualquer falta sem precisar reabrir toda a chamada:")

                    if qtd_ausentes_hoje == 0:
                        st.success("🎉 **100% DE PRESENÇA NESTA AULA!** Nenhum estudante ausente.")
                    else:
                        ...
                        for _, r_fal in alunos_faltosos_df.iterrows():
                            ...
                            with st.container(border=True):
                                ...
                                if c_fal2.button("🟢 Dar Presença", key=f"btn_quick_pres_{id_fal_al}_{data_ativa_str}_{v}", use_container_width=True):
                                    db.salvar_no_banco("DB_DIARIO_BORDO", [
                                        data_ativa_str, id_fal_al, nome_fal_al, turma_foco, "TRUE", "", "[PRESENÇA RETIFICADA EM 1-TOQUE]", "0,00"
                                    ])
                        ...
                        txt_copia_zap = f"📌 FALTAS ({turma_foco}) — AULA DO DIA {data_ativa_str}:\n" + "\n".join(lista_faltas_formatadas)
                        
                        with st.expander("📱 Copiar Lista Formatada para o WhatsApp da Secretaria", expanded=False):
                            st.code(txt_copia_zap, language=None)

            # ==============================================================
            # VISÃO 2: RADIOGRAFIA ANALÍTICA 360° (INTELIGÊNCIA EXECUTIVA)
            # ==============================================================
            elif visao_gestao_sel == "🧠 2. Radiografia Analítica 360°":
                st.markdown("#### 🧠 Radiografia Analítica da Turma & Diagnóstico SAEB")
                ...
                # 1. Termômetro Executivo em Bento Cards
                ...
                with st.container(border=True):
                    ...
                    k_r3.metric("📓 Vistos de Caderno (C1)", f"{taxa_vistos:.1f}%")
                    k_r4.metric("👥 Alunos Ativos", len(alunos_t))

                st.markdown("---")

                # 2. Distribuição por Níveis de Proficiência (Escala SAEB)
                st.markdown("##### 📈 Distribuição da Turma por Níveis de Proficiência (Padrão SAEB)")
                ...
                # 3. Radar Duplo de Risco: UTI Pedagógica x Radar de Evasão
                c_uti_col, c_eva_col = st.columns(2)

                with c_uti_col:
                    with st.container(border=True):
                        st.markdown("##### 🚑 UTI Pedagógica (< 6.0)")
                        st.caption("Estudantes que necessitam de intervenção e recomposição de notas:")
                        ...
                        if alunos_uti_list:
                            ...
                            st.dataframe(
                                ...
                                column_config={
                                    "Estudante": st.column_config.TextColumn(width="medium"),
                                    "Média Atual": st.column_config.NumberColumn(format="%.1f"),
                                    "Falta p/ 6.0": st.column_config.ProgressColumn("Precisa de", format="%.1f", min_value=0.0, max_value=6.0)
                                }
                            )
                        else:
                            st.success("🎉 Nenhum aluno na UTI de notas neste trimestre!")

                with c_eva_col:
                    with st.container(border=True):
                        st.markdown("##### 🚨 Radar de Evasão & Faltas Críticas")
                        ...
                        if alunos_evasao_list:
                            ...
                            st.success("✅ Nenhum estudante em risco de evasão!")

                # Botão de Exportação Oficial para Coordenação
                st.markdown("---")
                if st.button("🖨️ GERAR RELATÓRIO OFICIAL DA TURMA PARA A COORDENAÇÃO (DOCX)", type="primary", use_container_width=True, key=f"btn_exp_coord_hub_{v}"):
            ...
            # ==============================================================
            # VISÃO 3: SECRETARIA, MATRÍCULAS & CALENDÁRIO
            # ==============================================================
            else:
                st.markdown("#### ⚙️ Secretaria, Matrículas & Gestão de Calendário")
                
                modo_sec = st.pills(
                    "Operação Desejada:",
                    ["➕ Povoar e Cadastrar Alunos", "✏️ Edição em Cascata & Status", "📅 Gestor de Semanas & Recessos", "🏗️ Criar Turmas/Horários"],
                    default="➕ Povoar e Cadastrar Alunos",
                    key=f"pills_sec_sub_{v}"
                )
                st.markdown("---")

                if modo_sec == "➕ Povoar e Cadastrar Alunos":
                    t_dest = st.selectbox("Turma de Destino:", lista_turmas_segura, key=f"dest_pov_hub_{v}")
                    if t_dest:
                        t1_man, t2_lote = st.tabs(["✍️ Cadastro Individual", "📄 Importação em Lote (CSV)"])
                        ...
                        with t1_man:
                            with st.form("f_manual_povoar_hub"):
                                ...
                                if st.form_submit_button("💾 SALVAR ALUNO"):
                                    if not nome_a: st.error("Digite o nome do estudante.")
                                    else:
                                        ...
                                        if db.salvar_no_banco("DB_ALUNOS", [db.gerar_proximo_id(df_alunos), nome_a, t_dest, "ATIVO", perfil_str, "MANUAL"]):
                                            st.success(f"✅ {nome_a} cadastrado com sucesso!"); st.rerun()

                        with t2_lote:
                            st.info("💡 Cole a lista de alunos (NOME, PERFIL). Se o nome terminar com asterisco (*), será detectado como PEI.")
                            texto_lote = st.text_area("Cole os dados aqui:", height=180, placeholder="ADRIEL VINICIUS ALVES MARTINS,TÍPICO\nJOSE LEVI BRONZE SANTOS*,PEI - PENDENTE", key=f"txt_lote_hub_{v}")
                ...
                elif modo_sec == "✏️ Edição em Cascata & Status":
                    ...
                    if t_origem:
                        ...
                        if not alunos_opcoes.empty:
                            ...
                            if c_b1.button("📚 Def. Leitura", use_container_width=True, key=f"btn_diag_dl_h_{v}"):
                                db.atualizar_aluno_cascata(id_atual_ed, nome_atual_ed, t_origem, "DEFASAGEM LEITURA"); st.rerun()
                            if c_b2.button("🧮 Def. Matemática", use_container_width=True, key=f"btn_diag_dm_h_{v}"):
                            ...
                            if c_b4.button("👤 Típico (Limpar)", use_container_width=True, key=f"btn_diag_tip_h_{v}"):
                            ...
                            with st.form("form_edicao_cascata_hub"):
                                ...
                                if st.form_submit_button("💾 SALVAR ALTERAÇÕES EM CASCATA"):
                                    ...
                                    if db.atualizar_aluno_cascata(id_atual_ed, novo_nome, nova_turma, nova_nec):
                                        st.success("✅ Cadastro e Histórico propagados com sucesso!"); time.sleep(0.8); st.rerun()

                elif modo_sec == "📅 Gestor de Semanas & Recessos":
                    c_cal1, c_cal2 = st.columns([2, 1])
                    todas_semanas_cal = util.gerar_semanas()
                    semana_alvo_cal = c_cal1.selectbox("Selecione a Semana:", todas_semanas_cal, key=f"cal_sem_sel_hub_{v}")
                    status_semana_cal = c_cal2.selectbox("Status:", [
                    ...
                    motivo_semana_cal = st.text_input("Descrição / Observação:", placeholder="Ex: Recesso Junino / Semana de Provas Globais", key=f"cal_mot_hub_{v}")
                    ...
                    if st.button("💾 GRAVAR STATUS NO CALENDÁRIO", type="primary", use_container_width=True, key=f"btn_save_cal_hub_{v}"):
                        ...
                        st.success(f"✅ {sem_limpa_cal} configurada com sucesso!"); time.sleep(0.8); st.rerun()

                else:
                    with st.container(border=True):
                        c1, c2, c3 = st.columns(3)
                        ano_t_cad = c1.selectbox("Série/Ano:", [6, 7, 8, 9], index=0, key=f"ano_cad_hub_{v}")
                        letra_t_cad = c2.selectbox("Letra da Turma:", ["A", "B", "C", "D", "E"], key=f"letra_cad_hub_{v}")
                        ...
                        if st.button("💾 ALOCAR TURMA NA GRADE", type="primary", use_container_width=True, key=f"btn_save_turma_hub_{v}"):
                            if dias_aula_cad:
                                if db.salvar_no_banco("DB_TURMAS", [sigla_final_cad, nome_final_cad, turno_t_cad, " / ".join(dias_aula_cad), "N/A", "ATIVO"]):
                                    st.success(f"✅ Turma {sigla_final_cad} alocada com sucesso!"); time.sleep(0.8); st.rerun()

        # RENDERIZAÇÃO DO COCKPIT EM FRAGMENTO
