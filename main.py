...
# ==============================================================================
# MÓDULO: LABORATÓRIO DE PRODUÇÃO (CRIADOR V32.8) - DISTINÇÃO AULA 1/2
# ==============================================================================
if menu == "🧪 Criador de Aulas":
    ...
    tab_producao, tab_diagnostico, tab_trabalhos, tab_complementar, tab_acervo = st.tabs([
        "🚀 Produção (Aula 1/2)", "🔍 Diagnóstico Reverso", "📋 Engenharia de Trabalhos", "📚 Atividades Complementares", "📂 Acervo de Materiais"
    ])
        ...
        if "lab_temp" not in st.session_state:
            with st.container(border=True):
                ...
                if planos_ano.empty: st.error("❌ Nenhum plano encontrado.")
                    ...
                    # --- LÓGICA DE EXCLUSÃO DE OPÇÃO JÁ CRIADA ---
                    # Verifica no acervo se já existe Aula 1 ou Aula 2 para esta semana
                    materiais_da_semana = df_aulas[df_aulas['SEMANA_REF'] == sem_lab]['TIPO_MATERIAL'].tolist()
                    opcoes_alvo = ["Aula 1", "Aula 2"]
                    opcoes_filtradas = [opt for opt in opcoes_alvo if not any(opt in m for m in materiais_da_semana)]
                    
                    if not opcoes_filtradas:
                        st.success(f"✅ As duas aulas da **{sem_lab}** já foram produzidas!")
                        aula_alvo = None
                    else:
                        aula_alvo = st.radio("🎯 Alvo da Produção (Opções Disponíveis):", opcoes_filtradas, horizontal=True, key=f"prod_alvo_{v}")
                        ...
                        if st.button("💎 COMPILAR MATERIAL", use_container_width=True, type="primary"):
                            s_id = util.gerar_sosa_id("AULA", aula_alvo, sem_lab, ano_lab, "I")
                            st.session_state.sosa_id_atual = s_id
                            st.session_state.lab_meta = {"ano": ano_lab, "trimestre": "I Trimestre", "tipo": aula_alvo, "semana": sem_lab}
                            st.session_state.lab_temp = ai.gerar_ia("MESTRE_V24", f"GERAR {aula_alvo.upper()}. ID: {s_id}. PLANO: {plano_ref}. QTD: {qtd_q}. EXTRA: {instr_extra}")
                            st.rerun()
        else:
            # --- ÁREA DE EXIBIÇÃO (DENTRO DA ABA PRODUÇÃO) ---
            txt_base = st.session_state.lab_temp
            s_id = st.session_state.sosa_id_atual
            meta = st.session_state.lab_meta
            
            st.success(f"💎 {meta['tipo']} Gerada: **{s_id}**")
            st.button("🆕 VOLTAR / CRIAR OUTRA", on_click=reset_laboratorio, use_container_width=True)
            ...
            with t_gab: st.text_area("Gabarito:", ai.extrair_tag(txt_base, "GABARITO"), height=300)
            with t_pei:
                if st.button("✨ GERAR PEI"):
                    res_ia = ai.gerar_ia("ARQUITETO_PEI_V24", f"ADAPTE PARA PEI: {ai.extrair_tag(txt_base, 'ALUNO')}")
                    st.session_state.lab_pei, st.session_state.lab_gab_pei = ai.extrair_tag(res_ia, "PEI"), ai.extrair_tag(res_ia, "GABARITO_PEI")
                    st.rerun()
                if "lab_pei" in st.session_state:
                    c_p1, c_p2 = st.columns(2)
                    with c_p1: st.text_area("Material PEI:", st.session_state.lab_pei, height=400)
                    with c_p2: st.text_area("Gabarito PEI:", st.session_state.lab_gab_pei, height=400)

            with t_sync:
                if st.button("💾 EXECUTAR TRIPLE-SYNC", use_container_width=True, type="primary"):
                    with st.status("Sincronizando...") as status:
                        # NOME CLARO PARA O DRIVE: "AULA 1 - SEMANA 01 - 6º ANO"
                        nome_limpo = f"{meta['tipo'].upper()} - {meta['semana'].upper()} - {meta['ano']}º ANO"
                        
                        db.excluir_registro_com_drive("DB_AULAS_PRONTAS", s_id)
                        doc_alu = exporter.gerar_docx_aluno_v24(nome_limpo, ai.extrair_tag(txt_base, "ALUNO"), {"ano": f"{meta['ano']}º", "trimestre": "I"})
                        link_alu = db.subir_e_converter_para_google_docs(doc_alu, f"{nome_limpo}_ALUNO", modo="AULA")
                        
                        doc_prof = exporter.gerar_docx_professor_v25(nome_limpo, ai.extrair_tag(txt_base, "PROFESSOR"), {"ano": f"{meta['ano']}º", "semana": meta['semana'], "trimestre": "I"})
                        link_prof = db.subir_e_converter_para_google_docs(doc_prof, f"{nome_limpo}_PROF", modo="AULA")
                        
                        link_pei = "N/A"
                        if "lab_pei" in st.session_state:
                            doc_pei = exporter.gerar_docx_pei_v25(f"{nome_limpo}_PEI", st.session_state.lab_pei, {"ano": f"{meta['ano']}º", "trimestre": "I"})
                            link_pei = db.subir_e_converter_para_google_docs(doc_pei, f"{nome_limpo}_PEI", modo="AULA")
                        
                        if "https" in str(link_alu):
                            conteudo_banco = f"[SOSA_ID: {s_id}]\n{txt_base}\n\n[PEI]\n{st.session_state.get('lab_pei','N/A')}\n\n--- LINKS ---\nAluno({link_alu}) Prof({link_prof}) PEI({link_pei})"
                            db.salvar_no_banco("DB_AULAS_PRONTAS", [datetime.now().strftime("%d/%m/%Y"), meta['semana'], nome_limpo, conteudo_banco, f"{meta['ano']}º", link_alu])
                            status.update(label="✅ Sincronizado!", state="complete")
                            st.balloons(); time.sleep(1); reset_laboratorio()

    # --- ABA 5: ACERVO (DASHBOARD VISUAL) ---
    with tab_acervo:
        ...
        if not df_aulas.empty:
            ...
            for _, row in df_g.iloc[::-1].iterrows():
                ...
                with st.container(border=True):
                    c_t1, c_t2, c_t3, c_t4, c_t5 = st.columns([2.5, 1, 1, 1, 1])
                    ...
                    if l_alu: c_t2.link_button("📝 ALUNO", l_alu.group(1), use_container_width=True)
                    if l_prof: c_t3.link_button("👨‍🏫 PROF", l_prof.group(1), use_container_width=True)
                    if l_pei and "N/A" not in l_pei.group(1): c_t4.link_button("♿ PEI", l_pei.group(1), use_container_width=True)
                    else: c_t4.button("⚪ SEM PEI", disabled=True, use_container_width=True)
                    
                    if c_t5.button("🗑️ APAGAR", key=f"del_{row.name}", use_container_width=True):
