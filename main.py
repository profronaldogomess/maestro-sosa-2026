...
# ==============================================================================
# MÓDULO: LABORATÓRIO DE PRODUÇÃO (CRIADOR V33) - FOCO EM AULA 1/2 & DRIVE ORGANIZADO
# ==============================================================================
if menu == "🧪 Criador de Aulas":
    st.title("🧪 Laboratório de Produção Semiótica (V33)")
    st.markdown("---")
    ...
    with tab_producao:
        ...
        # SÓ MOSTRA O FORMULÁRIO SE NÃO HOUVER MATERIAL GERADO
        if "lab_temp" not in st.session_state or is_refinando_aula:
            ...
            with st.container(border=True):
                ...
                if planos_ano.empty:
                    ...
                    # ALVO APENAS AULA 1 OU AULA 2
                    aula_alvo = c3.radio("🎯 Alvo da Produção:", ["Aula 1", "Aula 2"], horizontal=True, key=f"alvo_{v}")
                    
                    plano_ref_row = planos_ano[planos_ano['SEMANA'] == sem_lab].iloc[0]
                    plano_raw = plano_ref_row['PLANO_TEXTO']
                    trim_ref = plano_ref_row['TRIMESTRE']

                    with st.container(border=True):
                        st.markdown(f"### 🔗 Sincronia PIP: {aula_alvo}")
                        col_p1, col_p2 = st.columns([1, 2])
                        qtd_q = col_p1.slider("Nº de Questões:", 1, 15, 5, key=f"q_{v}")
                        instr_extra = col_p2.text_input("Instruções Adicionais:", key=f"extra_{v}")
                        
                        if st.button("💎 COMPILAR MATERIAL DE ELITE", use_container_width=True, type="primary", key=f"btn_gen_{v}"):
                            s_id = util.gerar_sosa_id("AULA", ano_lab, trim_ref)
                            st.session_state.sosa_id_atual = s_id
                            # GUARDA METADADOS PARA O DRIVE
                            st.session_state.lab_meta = {
                                "ano": ano_lab, "trimestre": trim_ref, 
                                "semana": sem_lab, "aula": aula_alvo, "tipo": "AULA"
                            }
                            # Busca apenas a parte da aula correspondente no plano para a IA focar
                            foco_plano = ai.extrair_tag(plano_raw, aula_alvo.replace(" ", "_").upper())
                            prompt = f"GERAR {aula_alvo.upper()}. ID: {s_id}. CONTEXTO DO PLANO: {foco_plano}. QTD QUESTÕES: {qtd_q}. EXTRA: {instr_extra}"
                            st.session_state.lab_temp = ai.gerar_ia("MESTRE_V24", prompt)
                            st.rerun()
        else:
            # ÁREA DE EXIBIÇÃO (O FORMULÁRIO SUMIU)
            txt_base = st.session_state.lab_temp
            s_id = st.session_state.sosa_id_atual
            meta = st.session_state.lab_meta
            
            st.success(f"💎 {meta['aula']} Gerada: **{s_id}** ({meta['semana']})")
            if st.button("🆕 GERAR OUTRA AULA (LIMPAR ATUAL)", use_container_width=True): reset_laboratorio()

            # Extração prévia
            c_prof = ai.extrair_tag(txt_base, "PROFESSOR")
            c_alu = ai.extrair_tag(txt_base, "ALUNO")
            c_res = ai.extrair_tag(txt_base, "GABARITO") or ai.extrair_tag(txt_base, "RUBRICA")

            t_prof, t_alu, t_gab, t_pei, t_sync = st.tabs(["👨‍🏫 Professor", "📝 Aluno", "✅ Gabarito", "♿ PEI", "☁️ SINCRONIA"])
            
            with t_prof: st.text_area("Lousa:", c_prof, height=400, key=f"ed_prof_{v}")
            with t_alu: st.text_area("Folha:", c_alu, height=400, key=f"ed_alu_{v}")
            with t_gab: st.text_area("Respostas:", c_res, height=300, key=f"ed_res_{v}")
            
            with t_pei:
                if st.button("✨ GERAR MATERIAL PEI", use_container_width=True):
                    with st.spinner("Adaptando..."):
                        res_ia = ai.gerar_ia("ARQUITETO_PEI_V24", f"ADAPTE PARA PEI: {c_alu}")
                        st.session_state.lab_pei = ai.extrair_tag(res_ia, "PEI")
                        st.session_state.lab_gab_pei = ai.extrair_tag(res_ia, "GABARITO_PEI")
                        st.rerun()
                if "lab_pei" in st.session_state:
                    cp1, cp2 = st.columns(2)
                    cp1.text_area("Material PEI:", st.session_state.lab_pei, height=400)
                    cp2.text_area("Gabarito PEI:", st.session_state.lab_gab_pei, height=400)

            with t_sync:
                if st.button("💾 EXECUTAR TRIPLE-SYNC", use_container_width=True, type="primary"):
                    with st.status("Sincronizando...") as status:
                        # NOME ORGANIZADO PARA O DRIVE: SEMANA - AULA - ANO
                        nome_base = f"{meta['semana']} - {meta['aula']} - {meta['ano']}º Ano"
                        db.excluir_registro_com_drive("DB_AULAS_PRONTAS", s_id) # Limpa duplicatas
                        
                        doc_alu = exporter.gerar_docx_aluno_v24(nome_base, c_alu, {"ano": f"{meta['ano']}º", "trimestre": meta['trimestre']})
                        link_alu = db.subir_e_converter_para_google_docs(doc_alu, f"{nome_base}_ALUNO", modo="AULA")
                        
                        doc_prof = exporter.gerar_docx_professor_v25(nome_base, c_prof, {"ano": f"{meta['ano']}º", "semana": meta['semana'], "trimestre": meta['trimestre']})
                        link_prof = db.subir_e_converter_para_google_docs(doc_prof, f"{nome_base}_PROF", modo="AULA")
                        
                        link_pei = "N/A"
                        if "lab_pei" in st.session_state:
                            doc_pei = exporter.gerar_docx_pei_v25(f"{nome_base}_PEI", st.session_state.lab_pei, {"ano": f"{meta['ano']}º", "trimestre": meta['trimestre']})
                        ...
                        if "https" in str(link_alu):
                            ...
                            db.salvar_no_banco("DB_AULAS_PRONTAS", [datetime.now().strftime("%d/%m/%Y"), meta['semana'], f"{meta['aula']} - {meta['semana']}", conteudo_banco, f"{meta['ano']}º", link_alu])
    ...
    with tab_acervo:
        ...
        if not df_aulas.empty:
            f_ano_g = st.selectbox("Filtrar Série:", ["Todos", "6º", "7º", "8º", "9º"], key=f"filter_acervo_{v}")
            df_g = df_aulas.copy()
            ...
            for _, row in df_g.iloc[::-1].iterrows():
                ...
                sem_h = ai.extrair_tag(raw_c, "SEMANA")
                aula_h = ai.extrair_tag(raw_c, "AULA")
                
                with st.container(border=True):
                    c_t1, c_t2, c_t3, c_t4, c_t5, c_t6 = st.columns([1.8, 0.8, 0.8, 0.8, 0.8, 0.8])
                    c_t1.markdown(f"**{sem_h}**\n`{aula_h} | ID: {s_id_h}`")
                    ...
                    if link_alu: c_t2.link_button("📝 ALU", str(link_alu), use_container_width=True)
                    if link_prof: c_t3.link_button("👨‍🏫 PRF", str(link_prof), use_container_width=True)
                    if link_pei: c_t4.link_button("♿ PEI", str(link_pei), use_container_width=True)
                    else: c_t4.button("⚪ -", disabled=True, use_container_width=True)
                    
                    if c_t5.button("🔄", key=f"ref_{row.name}", help="Refinar"):
                    ...
                    if c_t6.button("🗑️", key=f"del_{row.name}", help="Apagar"):
                        if db.excluir_registro_com_drive("DB_AULAS_PRONTAS", s_id_h): st.rerun()
                    
                    with st.expander("👁️ Ver Estrutura"):
                        col1, col2 = st.columns(2)
                        col1.info("**Professor**"); col1.write(ai.extrair_tag(raw_c, "PROFESSOR"))
                        col2.success("**Aluno**"); col2.write(ai.extrair_tag(raw_c, "ALUNO"))
