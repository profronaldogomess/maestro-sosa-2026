...
# ==============================================================================
# MÓDULO: LABORATÓRIO DE PRODUÇÃO (CRIADOR) - ARQUITETURA V26.6 (ELITE)
# ==============================================================================
    ...
    with tab_producao:
        ...
        if "lab_temp" in st.session_state:
            ...
            cmd_refine = st.chat_input("Deseja ajustar algo? (Ex: 'Troque a Q3 por um desafio')")
            if cmd_refine:
            ...
            with t_lousa: ed_prof = st.text_area("Esquema de Lousa:", ai.extrair_tag(st.session_state.lab_temp, "PROFESSOR"), height=400, key=f"ed_prof_{v}")
            with t_folha: ed_alu = st.text_area("Folha do Aluno:", ai.extrair_tag(st.session_state.lab_temp, "ALUNO"), height=400, key=f"ed_alu_{v}")
    ...
    # --- ABA 2: ACERVO DE MATERIAIS (BOTÕES TRIPLOS) ---
    with tab_acervo:
        ...
        if not df_aulas.empty:
            ...
            for _, row in df_g.iloc[::-1].iterrows():
                with st.container(border=True):
                    st.markdown(f"**{row['TIPO_MATERIAL']}** ({row['SEMANA_REF']})")
                    
                    txt_full = str(row['CONTEUDO'])
                    # Extração de links individuais
                    ...
                    if l_alu: c_b1.link_button("📝 ALUNO", l_alu, use_container_width=True)
                    if l_prof: c_b2.link_button("👨‍🏫 PROF", l_prof, use_container_width=True)
                    if l_pei: c_b3.link_button("♿ PEI", l_pei, use_container_width=True)
                    else: c_b3.button("⚪ SEM PEI", disabled=True, use_container_width=True)
                    
                    if c_b4.button("🗑️ APAGAR", key=f"del_lab_{row.name}", use_container_width=True):
