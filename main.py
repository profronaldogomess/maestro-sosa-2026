...
def analisar_gabarito_vision(imagem_bytes):
    """
    MAESTRO VISION V5.1 - Protocolo de Tolerância Zero.
    Localizado no ai_engine.py para usar o 'client' global.
    """
    try:
        prompt = (
            "Você é um sensor óptico de alta sensibilidade. Analise a grade de questões.\n"
            "REGRAS DE OURO:\n"
            "1. MARCAÇÃO DUPLA: Se houver sinal de preenchimento em mais de um círculo na mesma linha, retorne 'X'.\n"
            "2. VAZIA: Se não houver marcação, retorne '?'.\n"
            "3. ÚNICA: Retorne a letra (A, B, C, D, E) apenas se houver exatamente um círculo pintado.\n"
            "Retorne APENAS um JSON puro: {'01': 'A', '02': 'X', ...}"
        )
        ...
        # O 'client' já está definido no topo do seu ai_engine.py
        res = client.models.generate_content(
