...
# ==============================================================================
    ...
    st.markdown("---")
    st.caption("💡 **Guia de Comando:** Transforme seus planejamentos em materiais físicos de alta densidade. Aulas regulares usam a Forja em 5 Fases. Projetos e Listas usam a geração direta.")
    ...
    # 🚨 FILTRO DE ILUSÃO DE ÓTICA (RENDERIZAÇÃO LATEX E GEOGEBRA)
    ...
    # ==============================================================================
    # 🖥️ EDITOR CLÁSSICO (PARA PROJETOS, LISTAS E RECOMPOSIÇÃO)
    # ==============================================================================
    if "lab_temp" in st.session_state:
        ...
        modo_leitura = c_tog1.toggle("👁️ Ativar Modo Leitura (Renderizar Matemática)", value=False)
        ...
        with t_prof: 
            ...
            else: ed_prof = st.text_area("Lousa/Mediação:", val_prof, height=450, key=f"ed_prof_reg_{v}")
        
        with t_alu: 
            if modo_leitura: st.markdown(preparar_para_leitura(val_alu))
            else: ed_alu = st.text_area("Folha/Roteiro:", val_alu, height=450, key=f"ed_alu_reg_{v}")
        ...
        with t_pei_tab: 
            ...
            else: ed_pei = st.text_area("PEI (Obrigatório):", val_pei, height=400, key=f"ed_pei_reg_{v}")
    ...
    # ==============================================================================
    # 👨‍🏫 FASE 2: O TRATADO DIDÁTICO (GUIA DO PROFESSOR) - FORJA DE AULAS
    # ==============================================================================
    elif fa['fase'] == 2:
        if st.button("🔄 Cancelar e Voltar ao Início"): reset_laboratorio()
        st.progress(2 / 5.0, text="Fase 2 de 5")
        st.markdown("---")
        
        st.markdown("### 👨‍🏫 Fase 2: O Tratado Didático (Teoria)")
        st.caption("A IA gerará a explicação formal, o gancho glocalizado e os comandos do GeoGebra. Revise e aprove antes de gerar os exercícios.")
        
        if not fa['teoria']:
            with st.spinner("Forjando o Tratado Didático..."):
                prompt_teoria = (
                ...
                if fa['links_web'].strip():
                    prompt_teoria += f"\n🚨 USE ESTES LINKS COMO BASE TEÓRICA PRINCIPAL:\n{fa['links_web']}\n"
            ...
            modo_leitura = st.toggle("👁️ Ativar Modo Leitura (Renderizar Matemática e GeoGebra)", value=True)
            
            if modo_leitura:
                st.info(preparar_para_leitura(fa['teoria']))
            else:
    ...
    # ==============================================================================
        ...
        if st.button("🔄 Cancelar e Voltar ao Início"): reset_laboratorio()
        st.progress(3 / 5.0, text="Fase 3 de 5")
        st.markdown("---")
        
        st.markdown("### 📝 Fase 3: A Missão do Aluno (Exercícios Regulares)")
        st.caption("A IA criará as questões discursivas baseadas estritamente na teoria que você acabou de aprovar.")
        
        if not fa['reg_q']:
            with st.spinner(f"Forjando {fa['qtd_q']} questões discursivas..."):
                prompt_exercicios = (
                    f"SÉRIE ALVO: {fa['info']['ano']}º Ano.\n"
            ...
            modo_leitura = st.toggle("👁️ Ativar Modo Leitura", value=True)
            
            t_alu, t_gab = st.tabs(["📝 Folha do Aluno", "✅ Gabarito"])
            with t_alu:
                if modo_leitura: st.info(preparar_para_leitura(fa['reg_q']))
                else: fa['reg_q'] = st.text_area("Edição do Aluno:", value=fa['reg_q'], height=400)
            with t_gab:
    ...
    # ==============================================================================
        ...
        if st.button("🔄 Cancelar e Voltar ao Início"): reset_laboratorio()
        st.progress(4 / 5.0, text="Fase 4 de 5")
        st.markdown("---")
        
        st.markdown("### ♿ Fase 4: O Escudo Inclusivo (Adaptação PEI)")
        st.caption("A IA converterá as questões regulares em formato de múltipla escolha com apoios visuais.")
        
        if not fa['pei_q']:
            with st.spinner("Adaptando questões para o formato PEI..."):
                prompt_pei = (
                    f"QUANTIDADE DE QUESTÕES: {fa['qtd_q']}.\n"
            ...
            t_pei, t_gab_pei = st.tabs(["♿ Folha PEI", "✅ Gabarito PEI"])
            with t_pei:
                if modo_leitura: st.warning(preparar_para_leitura(fa['pei_q']))
                else: fa['pei_q'] = st.text_area("Edição PEI:", value=fa['pei_q'], height=400)
            with t_gab_pei:
    ...
    # ==============================================================================
        ...
        if st.button("🔄 Cancelar e Voltar ao Início"): reset_laboratorio()
        st.progress(5 / 5.0, text="Fase 5 de 5")
        st.markdown("---")
        
        st.markdown("### 💾 Fase 5: Compilação e Sincronia")
        ...
        if st.button("🚀 SALVAR MATERIAL E SINCRONIZAR NO DRIVE", type="primary", use_container_width=True):
            with st.status("Sincronizando Ativos de Elite...") as status:
                import exporter
    ...
    # ==============================================================================
        ...
        # --- ABA 1: PRODUÇÃO DE AULAS (FORJA SEMIÓTICA) ---
        with tab_producao:
                    ...
                    if not semanas_opcoes:
                            ...
                            if not opcoes_disponiveis:
                                ...
                                with col_config2:
                                    if is_logistica:
                                        ...
                                        qtd_q_prod = st.slider("Nº de Questões (PEI/Regular):", 1, 20, 5, key=f"prod_q_{v}")
                                ...
                                with st.expander(f"👁️ Roteiro Herdado para {aula_alvo_prod}", expanded=False):
                                    st.info(f"📍 **Base Didática:** {paginas_aula}\n\n{roteiro_especifico}")
                        ...
                        if opcoes_disponiveis:
                            if is_logistica:
                                ...
                                # 🚨 INJEÇÃO DE LINKS DA WEB NO CRIADOR DE AULAS
                                st.markdown("---")
                                st.markdown("#### 🌐 Enriquecimento Web (Opcional)")
                                st.caption("Cole links de artigos ou sites (ex: Brasil Escola) para a IA usar como base teórica desta aula.")
                                
                                links_pre_preenchidos = ""
                                if "http" in base_herdada:
                                    links_pre_preenchidos = "\n".join(re.findall(r'(https?://[^\s]+)', base_herdada))
                                
                                links_web_aula = st.text_area("Links da Web (um por linha):", value=links_pre_preenchidos, height=100, key=f"links_web_aula_{v}")

                                st.markdown("<br>", unsafe_allow_html=True)
                                if st.button("🧠 INICIAR FORJA SEMIÓTICA (FASE 2)", use_container_width=True, type="primary"):
                                    fa['info'] = {
                                        "ano": ano_lab, 
                                        "semana_ref": sem_lab, 
                                        "aula_alvo": aula_alvo_prod,
                                        "roteiro": roteiro_especifico,
                                        "habilidade": ai.extrair_tag(plano_txt, "HABILIDADE_BNCC"),
                                        "objetivos": ai.extrair_tag(plano_txt, "OBJETIVOS_ENSINO"),
                                        "base": base_herdada
                                    }
...
# ==============================================================================
    ...
    st.caption("💡 **Guia de Comando:** Visão analítica da jornada do aluno. Use este painel em reuniões de pais para justificar médias mostrando a composição exata das notas, a evolução visual nas provas e o engajamento em sala.")
    st.markdown("---")
...
Qualquer dúvida, estou à disposição! Um abraço! 🚀"""
        ...
        # --- BLOCO 2: AUTÓPSIA DE AVALIAÇÕES (CLEAN) ---
        st.markdown(f"### 🎯 2. Histórico de Avaliações (Scanner)")
        st.caption("Relação de todas as provas e testes realizados pelo aluno. Clique no link para ver a foto do gabarito.")
        ...
        if not diag_alu_f.empty:
            ...
            for _, row_av in diag_ordenado.iterrows():
                ...
                # Limpeza do nome da prova para ficar elegante
                ...
                dados_av.append({
                    "Tipo": icone,
                    "Avaliação": nome_limpo_av,
                    "Data": data_av,
                    "Nota": nota_av,
                    "Status": status_av,
                    "Evidência": link_foto if "http" in link_foto else None
                })
                
            st.dataframe(
                pd.DataFrame(dados_av),
                use_container_width=True, hide_index=True,
                column_config={
        ...
        # ==============================================================================
        # ⚖️ NOVO: TRIBUNAL DE RECURSOS E DEFENSORIA
        # ==============================================================================
        st.markdown(f"### ⚖️ Tribunal de Recursos (Atendimento aos Pais)")
        st.caption("Use este painel para analisar questões contestadas, gerar evidências visuais e redigir respostas automáticas para o WhatsApp.")
        
        with st.container(border=True):
            if not diag_alu_f.empty:
                # 1. Selecionar a Avaliação
                ...
                if av_contestada:
                    ...
                    if not df_prova_trib.empty:
                        ...
                        # Montar dicionário do gabarito
                        matches_gab = re.findall(r"(\d+)[\s\.\)\-:]+([A-E])", gab_raw_trib.upper())
                        if matches_gab:
                            gab_oficial_trib = {int(num): letra for num, letra in matches_gab}
                        else:
                        ...
                        with st.container(border=True):
                            ...
                            if "http" in link_foto_trib:
                                c_ev2.link_button("📸 Ver Foto do Gabarito", link_foto_trib, use_container_width=True)
                            ...
                            with st.container(border=True):
                                ...
                                if st.button("💾 Confirmar Correção e Recalcular Média", type="primary"):
                                    with st.spinner("Corrigindo gabarito e recalculando boletim..."):
                                        ...
                                        if q_num_trib - 1 < len(novas_respostas):
                                            novas_respostas[q_num_trib - 1] = nova_letra
                                        else:
                                            novas_respostas.append(nova_letra)
                                            
                                        # 2. Recalcular Nota da Prova
                                        ...
                                        # 3. Atualizar DB_GABARITOS_ALUNOS
                                        try:
                                            wb = db.conectar()
                                            ...
                                            st.error(f"Erro ao atualizar gabarito: {e}")
                                            
                                        # 4. Atualizar DB_NOTAS (Recalcular Média)
                                        try:
                                            ...
                                            for i, row in enumerate(dados_notas):
                                                if i > 0 and db.limpar_id(row[0]) == id_alu and row[3] == trim_b:
                                                    ...
                                                    if r_n > 0:
                                                        nova_media_final = max(nova_media_final, r_n)
                                                        
                                                    ws_notas.update_cell(i+1, 9, util.sosa_to_str(nova_media_final))
                                                    break
                                        except Exception as e:
                                            st.error(f"Erro ao atualizar boletim: {e}")
                                            
                                        st.cache_data.clear()
                                        
                                        # 5. Gerar Mensagem de Retratação
                        ...
                        st.warning("A prova original não foi encontrada no acervo para realizar a perícia.")
            else:
                pass # Oculta se não houver avaliações
        ...
        st.caption("Acompanhe a trajetória das notas do aluno ao longo do tempo.")
        with st.container(border=True):
            if not diag_alu_f.empty:
                # Prepara os dados para o gráfico
                dados_grafico = []
                for _, row_av in diag_ordenado.iloc[::-1].iterrows(): # Inverte para ordem cronológica
                    av_id_bruto = row_av['ID_AVALIACAO']
                    nota_av = util.sosa_to_float(row_av['NOTA_CALCULADA'])
                    data_av = row_av['DATA']
                    ...
                    # Ignora faltas no gráfico para não distorcer a linha
                ...
                if dados_grafico:
                    ...
                    fig = px.line(
                        ...
                        text="Nota",
                        markers=True,
                        title="Curva de Aprendizagem (Notas em Avaliações)",
                        hover_data=["Data"]
                    )
                    ...
                    # Adiciona uma linha de meta (Média 6.0)
                    fig.add_hline(y=6.0, line_dash="dash", line_color="red", annotation_text="Média (6.0)", annotation_position="bottom right")
                    
                    st.plotly_chart(fig, use_container_width=True)
                else:
                    st.info("O aluno faltou a todas as avaliações deste período.")
            else:
                st.info("📭 Aguardando avaliações escaneadas para gerar o gráfico de evolução.")
...
# ==============================================================================
    ...
    st.caption("💡 **Guia de Comando:** Defina a data de congelamento para fechar as notas sem perder os vistos futuros. O sistema calcula tudo, separa quem está 'Quase Lá' e gera as etiquetas para colar nas provas.")
    st.markdown("---")
            ...
            if st.button("💾 SALVAR NOTAS E SINCRONIZAR BOLETIM", type="primary", use_container_width=True):
                with st.status("Sincronizando registros no Banco de Dados...") as status:
                    ...
                    if not df_diario_turma.empty:
                        ...
                        data_ancora = data_limite.strftime("%d/%m/%Y")
