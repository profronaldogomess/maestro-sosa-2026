...
# ==============================================================================
# MÓDULO: PLANEJAMENTO ESTRATÉGICO (PONTO ID) - V2026.MASTER
# ==============================================================================
if menu == "📅 Planejamento (Ponto ID)":
    st.title("Engenharia de Planejamento (Ponto ID)")
    st.caption("Arquitetura estratégica da semana. O sistema automatiza a burocracia, gerencia a matriz do seu CSV e alimenta o Criador de Aulas.")
    st.markdown("---")
        ...
        if tipo_semana in ["Semana de Provas Oficiais (Global)", "Devolutiva de Resultados & Recuperação"]:
            with st.container(border=True):
                ...
                if "Provas" in tipo_semana:
                    texto_padrao = f"Avaliação Global. Ocorrerão provas de diversas disciplinas conforme calendário da coordenação."
                    aula_1_txt = "Organização das fileiras. Leitura das instruções gerais. Aplicação do instrumento avaliativo."
                    aula_2_txt = "Continuação da aplicação ou recolhimento dos instrumentos." if "1 Aula" not in carga_horaria else "N/A"
                    aula_sab_txt = "Plantão tira-dúvidas ou exames pendentes." if "3 Aulas" in carga_horaria else "N/A"
                else:
                    texto_padrao = f"Análise de Erros e Recuperação Paralela. Foco nos tópicos com menor índice de acerto."
                    aula_1_txt = "Entrega dos resultados. Correção comentada no quadro das questões com menor acerto."
                    aula_2_txt = "Aplicação da Recuperação Paralela para elegíveis." if "1 Aula" not in carga_horaria else "N/A"
            ...
            with st.container(border=True):
                ...
                if tipo_semana != "Aula Aberta (Dinâmicas e Eventos)":
                    ...
                    else: st.warning(f"Nenhum material encontrado no acervo.")
                else:
                    ativo_selecionado = st.text_input("Nome do Evento/Dinâmica:", placeholder="Ex: Palestra sobre Educação Financeira")

                diretriz_logistica = st.text_area("Diretriz Logística:", placeholder="Ex: Os alunos terão 50 minutos para realizar a prova...")
            ...
            with st.container(border=True):
                ...
                st.caption(f"ℹ️ **Carga Horária Selecionada:** {carga_horaria}. A IA distribuirá a Teoria e os Exercícios dinamicamente.")
                ...
                if modo_p == "Manual (Matriz)":
                    ...
                    recorte_livro_texto = st.text_area(
                        "Cole anotações extras ou exercícios autorais:",
                        placeholder="Cole aqui textos ou exercícios extras do seu caderno...",
                        height=100,
            ...
            with st.popover("⚙️ Ajustes Finos e Diretrizes de Aula (Opcional)", use_container_width=True):
                st.caption("Especifique o que deseja em cada aula para direcionar a IA.")
                if "1 Aula" in carga_horaria:
                    foco_a1 = st.text_area("Foco da Única Aula:", placeholder="Ex: Explicar conceito e resolver exercícios...", height=80)
                ...
                with st.status("🚀 Iniciando Protocolo de Planejamento...", expanded=True) as status:
                    ...
                    if "1 Aula" in carga_horaria:
                        diretriz_carga_promp = (
                            ...
                            "- Concentre TODA a explicação da teoria e os exercícios na AULA 1.\n"
                            "- As tags [AULA_2] e [SABADO_LETIVO] DEVEM ser 'N/A (Carga horária de 1 Aula)'."
                        )
                    elif "2 Aulas" in carga_horaria:
                        diretriz_carga_promp = "CARGA HORÁRIA: 2 AULAS NA SEMANA. Distribua teoria e exercícios nas duas aulas.\n- Tag [SABADO_LETIVO] deve ser 'N/A'."
                    else:
                        diretriz_carga_promp = "CARGA HORÁRIA: 3 AULAS NA SEMANA. Distribua o conteúdo na AULA 1, AULA 2 e SÁBADO LETIVO."
    ...
    def renderizar_mesa_lapidacao_plano():
        if "p_temp" in st.session_state:
            ...
            with tab_curriculo:
                ed_hab = st.text_input("Habilidade/Competência:", ai.extrair_tag(txt_bruto, "HABILIDADE_BNCC") or ai.extrair_tag(txt_bruto, "COMPETENCIA_GERAL"), key=f"frag_hab_{v}")
                ...
                ed_espec = st.text_area("Conteúdos Específicos:", ai.extrair_tag(txt_bruto, "CONTEUDOS_ESPECIFICOS") or txt_bruto, height=150, key=f"frag_espec_{v}")
                ed_objs = st.text_area("Objetivos de Aprendizagem:", ai.extrair_tag(txt_bruto, "OBJETIVOS_ENSINO"), height=150, key=f"frag_objs_{v}")
            
            with tab_roteiro:
                c_a1, c_a2, c_a3 = st.columns(3)
                ed_a1 = c_a1.text_area("AULA 1:", ai.extrair_tag(txt_bruto, "AULA_1"), height=400, key=f"frag_a1_{v}")
                ed_a2 = c_a2.text_area("AULA 2:", ai.extrair_tag(txt_bruto, "AULA_2"), height=400, key=f"frag_a2_{v}")
                ed_sab = c_a3.text_area("SÁBADO LETIVO:", ai.extrair_tag(txt_bruto, "SABADO_LETIVO"), height=400, key=f"frag_sab_{v}")
    ...
    with tab_producao:
        ...
        st.caption("Planos aprovados aguardando a geração dos materiais físicos.")
                ...
                if is_corrupted:
                    ...
                    def renderizar_relocador_fragmento():
                        with st.expander("🔄 Mover este Plano e Aulas para Outra Semana", expanded=False):
                            st.info("💡 Altera a semana do plano e das aulas sem apagar os arquivos no Drive.")
                            ...
                            if not semanas_livres_reloc:
                                ...
                                if st.button("🚀 CONFIRMAR MUDANÇA DE SEMANA EM CASCATA", type="primary", use_container_width=True, key=f"btn_reloc_exe_{sel_h.replace(' ','')}"):
                                    with st.spinner("Transferindo plano e aulas..."):
                ...
                with st.spinner("Minerando planos e compilando documento..."):
                    ...
                    if df_matriz_trim.empty:
                        ...
                        for txt in planos_trim['PLANO_TEXTO'].dropna():
                            ...
                            if "tecnologia" in aula1 or "geogebra" in aula1: metodologias.add("Uso de recursos tecnológicos")
...
# ==============================================================================
    ...
    if "forja_aula" not in st.session_state:
        st.session_state.forja_aula = {
            ...
            'teoria': '', 'reg_q': '', 'reg_gab': '', 'pei_1': '', 'pei_3': '', 'pei_gab': '', 'nome_base': ''
        }
        ...
        if "lab_temp" in st.session_state:
            ...
            if fa['fase'] == 1:
                        ...
                        if planos_ano.empty:
                            ...
                            if not semanas_opcoes:
                                ...
                                if not opcoes_disponiveis:
                                    ...
                                    # 🚨 CAIXA DE RECORTE DO LIVRO DIDÁTICO / ANCORAGEM REFORÇADA MULTI-INTERVALO
                                    st.markdown("##### 📖 Ancoragem no Livro Didático & Autonomia Docente")
                                    
                                    col_p1, col_p2 = st.columns(2)
                                    pags_teo_lab = col_p1.text_input("📘 Páginas de Teoria (Aula 1):", placeholder="Ex: 184-186, 189", key=f"pags_teo_lab_{v}", help="Aceita intervalos e páginas avulsas (ex: 184-186, 189)")
                                    pags_ex_lab = col_p2.text_input("📝 Páginas de Exercícios (Aula 2):", placeholder="Ex: 187-188, 190-192", key=f"pags_ex_lab_{v}", help="Aceita intervalos e páginas avulsas (ex: 187-188, 190-192)")
                                    ...
                                    recorte_exercicios_livro = st.text_area(
                                        ...
                                        placeholder="Cole aqui exercícios ou textos do seu caderno/provas para somar com o livro didático...",
                                        height=100,
                                    ...
                                    # Consolidação do pacote completo para enviar à IA
                                    pacote_recorte_aula = ""
                    ...
                    with st.container(border=True):
                        ...
                        if puxar_memoria_lista:
                            ...
                            if memoria_lista_txt:
                                ...
                                st.warning("Ainda não há aulas concluídas no Diário para este trimestre. Selecione as aulas manualmente abaixo:")
                        ...
                        if st.button("🚀 INICIALIZAR LINHA DE MONTAGEM DE LISTA", use_container_width=True, type="primary"):
                            if not contexto_aulas.strip() and not aulas_sel:
                                st.error("⚠️ Selecione pelo menos um tema de aula ou ative o resgate automático de memória.")
                            else:
                ...
                if fa.get('tipo_material') == 'AULA':
                                    ...
                                    for idx_lote, q_data in enumerate(questoes_retornadas):
                                        if idx_lote < len(micro_lote):
                                            ...
                                            if item['suporte_visual'] and "[ PROMPT IMAGEM:" not in enunciado:
                                                enunciado += f"\n\n[ PROMPT IMAGEM: A4 portrait math worksheet, line art, black and white diagram of {item['tema']}. All text in Portuguese. ]"
                    ...
                    modo_leitura_list = st.toggle("👁️ Modo Leitura (Renderizar LaTeX)", value=True, key="read_list")
                    ...
                    for i, item in enumerate(fa['mapa_lista']):
                        ...
                        with st.expander(f"Item {item['q']:02d} | Gabarito: {item['gabarito']} ({label_status})", expanded=is_expanded):
                            if item['status'] == 'pendente':
                                ...
                                if col_b1.button(f"Aprovar Item {item['q']}", type="primary", key=f"btn_apr_l_{i}", use_container_width=True):
                    ...
                    if todas_aprovadas:
                        ...
                        if st.button("Avançar para a Cartilha PEI", type="primary", use_container_width=True):
                            fa['fase'] = 3; st.rerun()
                ...
                if not fa['reg_q']:
                    ...
                    modo_leitura_ex = st.toggle("👁️ Visualização Real", value=True, key="tog_ex")
                    t_q, t_g = st.tabs(["Folha do Aluno", "Gabarito"])
                    with t_q:
                        if modo_leitura_ex: st.markdown(preparar_para_leitura(fa['reg_q']))
                        else: fa['reg_q'] = st.text_area("Exercícios (Aluno):", value=fa['reg_q'], height=300)
                    with t_g:
                        if modo_leitura_ex: st.markdown(preparar_para_leitura(fa['reg_gab']))
                        else: fa['reg_gab'] = st.text_area("Resoluções comentadas:", value=fa['reg_gab'], height=200)
                    
                    if st.button("Aprovar Exercícios e Avançar", type="primary", use_container_width=True):
                        fa['fase'] = 4; st.rerun()
                ...
                if not fa['pei_1']:
                    with st.spinner("Adaptando para o formato inclusivo..."):
                        prompt_pei = f"Adapte as questões abaixo para PEI N1 e PEI N3:\n{fa['reg_q']}\n🚨 USE AS TAGS [PEI_NIVEL_1], [PEI_NIVEL_3] e [GABARITO_PEI]"
                    ...
                    t_p1, t_p3 = st.tabs(["🔵 PEI Nível 1", "🔴 PEI Nível 3"])
                    with t_p1: fa['pei_1'] = st.text_area("Nível 1:", value=fa['pei_1'], height=300)
                    with t_p3: fa['pei_3'] = st.text_area("Nível 3:", value=fa['pei_3'], height=300)
                    
                    if st.button("Aprovar PEI e Avançar", type="primary", use_container_width=True):
                        fa['fase'] = 5; st.rerun()
                ...
                nome_arq = st.text_input("Nome do Material:", value=nome_sugerido)
...
# ==============================================================================
    ...
    with tab_forja:
        if 1 < f['fase'] < 6:
            ...
            if st.button("Descartar e Voltar ao Início", use_container_width=True, key=f"btn_disc_av_{v}"): reset_forja()
            st.markdown("---")
        ...
        if f['fase'] == 1:
            ...
            # 🚨 UI NOVA GERAÇÃO: ST.PILLS PARA ABORDAGEM CURRICULAR
            modo_arq = st.pills(
                "Selecione a Abordagem Curricular:", 
            ...
            if "Inédita" in modo_arq or "Sonda" in modo_arq:
                with st.container(border=True):
                    ...
                    # 🚨 UI NOVA GERAÇÃO: POPOVER PARA PARÂMETROS TRI E RIGOR COGNITIVO
                    v_total = 3.0 if "Inédita" in modo_arq else 10.0
                    qtd_q = 10
                    tipo_av = "SONDA_DE_PROFICIÊNCIA" if "Sonda" in modo_arq else "Teste"
                    perfil_rigor = "⚖️ Padrão (Balanceado ENEM)"

                    with st.popover("⚙️ Ajustar Valor, Nº de Questões e Rigor TRI", use_container_width=True):
                        st.caption("Ajuste a pontuação e a quantidade de itens da prova.")
                        c_p1, c_p2 = st.columns(2)
                        v_total = c_p1.number_input("Valor da Prova:", 0.0, 10.0, v_total, step=0.5, key=f"v_tot_pop_{v}")
                        qtd_q = c_p2.number_input("Quantidade de Questões:", 1, 20, qtd_q, key=f"qtd_q_pop_{v}")
                        
                        if "Inédita" in modo_arq:
                            tipo_av = st.selectbox("Tipo de Instrumento:", ["Teste", "Prova", "Recuperação Paralela", "Recuperação Final"], key=f"tipo_inst_pop_{v}")
                        perfil_rigor = st.selectbox("Perfil de Rigor Cognitivo (TRI):", ["⚖️ Padrão (Balanceado ENEM)", "🚀 Alta Performance (OBMEP/Avançado)", "🧱 Foco em Fixação (Acessível)"], key=f"rigor_pop_{v}")

                with st.container(border=True):
                    st.markdown("#### Seleção Híbrida de Fontes & Questões Espelho")
                    st.caption("ℹ️ **Autonomia Total:** Combine matérias do Acervo, fatiamento do Livro Didático e injeção de texto manual.")
                    
                    fontes_ativas = st.pills(
                        "Escolha as Fontes de Dados para a Prova:", 
                        ["📚 Acervo de Aulas SOSA", "📖 Recorte do Livro (PDF)", "✍️ Injeção Auxiliar (Professor)"], 
                        default=["📖 Recorte do Livro (PDF)"],
                    ...
                    # FONTE 2: LIVRO DIDÁTICO FATIADO
                    if "📖 Recorte do Livro (PDF)" in fontes_ativas:
                        livros_av_disp = df_materiais[df_materiais['TIPO'].str.contains(str(ano_av), na=False)]['NOME_ARQUIVO'].tolist()
                        ...
                        if sel_livro_av:
                            ...
                            pags_ex_av = c_p_av2.text_input("📝 Páginas de Exercícios:", placeholder="Ex: 187-188, 190", key=f"pags_ex_av_{v}")
                            ...
                            if list_p_av_teo or list_p_av_ex:
                                with st.spinner("🔍 Fatiando páginas do livro para a prova..."):
                    ...
                    if txt_av_teo_ext or txt_av_ex_ext or recorte_provas_livro.strip():
                        with st.expander("👁️ MESA DE INSPEÇÃO DA PROVA (CONFERIR BASE DA IA)", expanded=True):
                            ...
                            with t_av_i1:
                                if txt_av_teo_ext: st.text_area("Teoria Lida:", txt_av_teo_ext, height=150, disabled=True)
                                else: st.info("Nenhuma página de teoria fatiada.")
                            with t_av_i2:
                                if txt_av_ex_ext: st.text_area("Exercícios Lidos:", txt_av_ex_ext, height=150, disabled=True)
                                else: st.info("Nenhuma página de exercício fatiada.")
                            with t_av_i3:
                                if recorte_provas_livro.strip(): st.text_area("Texto Auxiliar Autorizado:", recorte_provas_livro, height=150, disabled=True)
                                else: st.info("Nenhum texto auxiliar digitado.")
                ...
                if st.button("🚀 Gerar Matriz de Questões Ancoradas no Livro/Lousa", type="primary", use_container_width=True, key=f"btn_gen_matriz_{v}"):
                ...
                with st.container(border=True):
                    st.markdown("#### Parâmetros de Clonagem / 2ª Chamada")
                    c_cl1, c_cl2 = st.columns([1, 2])
                    ano_clone = c_cl1.selectbox("Série:", [6, 7, 8, 9], index=0, key=f"ano_clone_sel_{v}")
                    ...
                    prova_base_sel = c_cl2.selectbox("Prova de Origem:", [""] + opcoes_provas, key=f"p_base_sel_{v}")
                ...
                if prova_base_sel:
                    ...
                    if st.button(f"Iniciar Processo de Clonagem", type="primary", use_container_width=True, key=f"btn_clone_exe_{v}"):
                ...
                with st.container(border=True):
                    ...
                    ano_rec = c_rec1.selectbox("Série:", [6, 7, 8, 9], index=0, key=f"ano_rec_sel_{v}")
                    trim_rec = c_rec2.selectbox("Trimestre:", ["I Trimestre", "II Trimestre", "III Trimestre"], key=f"trim_rec_sel_{v}")
                    ...
                    provas_base_sel = st.multiselect("Instrumentos Originais de Diagnóstico:", opcoes_provas_rec, max_selections=2, key=f"p_base_rec_{v}")
                
                if len(provas_base_sel) > 0:
                    c_p1, c_p2 = st.columns(2)
                    gerar_n1 = c_p1.checkbox("Gerar Adaptação PEI Leve", value=True, key=f"chk_n1_rec_{v}")
                    gerar_n2 = c_p2.checkbox("Gerar Adaptação PEI Moderada", value=True, key=f"chk_n2_rec_{v}")
                    
                    if st.button("Forjar Instrumento de Recuperação", type="primary", use_container_width=True, key=f"btn_rec_exe_{v}"):
        ...
        # --- FASE 4: CUSTÓDIA E FINALIZAÇÃO ---
            ...
            nome_arq = st.text_input("Identificador Técnico (Cofre Digital):", value=nome_sugerido, key=f"nome_arq_f4_{v}")
            
            gerar_variante = st.checkbox("Gerar Variante Tipo B (Embaralhada)", value=False, key=f"chk_var_f4_{v}")
            
            if st.button("Finalizar e Gravar Ativos no Drive", type="primary", use_container_width=True, key=f"btn_sync_f4_av_{v}"):
        ...
        # --- FASE 5: AÇÕES PÓS-PROVA ---
            ...
            if c_pos1.button("Gerar Material de Revisão", use_container_width=True, key=f"btn_gen_rev_{v}"):
            ...
            if c_pos2.button("Gerar Caderno de 2ª Chamada", use_container_width=True, key=f"btn_gen_2a_{v}"):
                with st.spinner("Construindo..."):
    ...
    # ==============================================================================
    # ABA 2: ACERVO DE PROVAS (COM ST.BADGE E ANÁLISE DE DISTRATORES)
    # ==============================================================================
    with tab_acervo_av:
        ...
        if df_exames.empty:
            ...
            for idx_av, (_, row) in enumerate(df_exames.iterrows()):
                with st.container(border=True):
                    ...
                    st.markdown(f"##### {identificador}")
                    st.caption(f"Série: {row['ANO']} | Valor: {val_ex if val_ex else '10.0'} pts | Status: 🔒 COFRE DIGITAL DRIVE")
                    ...
                    else: c_b5.button("Sem Guia", disabled=True, use_container_width=True, key=f"no_prof_{row.name}_{idx_av}_{v}")
                    
                    if c_b6.button("✏️ Refinar", key=f"ref_av_h_{row.name}_{idx_av}_{v}", use_container_width=True):
...
# ==============================================================================
# MÓDULO: CENTRAL DE INTELIGÊNCIA DE RESULTADOS (CIR) - V2026.MASTER
# ==============================================================================
elif menu == "📸 Scanner de Gabaritos":
    st.title("Central de Inteligência de Resultados (CIR)")
    st.caption("Mesa de triagem de exames, espelho em split-screen, autópsia com Frequência por Alternativa e Perícia de Distratores TRI.")
    st.markdown("---")
    ...
    # ==============================================================================
    # ABA 1: MESA DE CORREÇÃO (COM ST.PILLS)
    # ==============================================================================
    with tab_correcao:
        # 🚨 UI NOVA GERAÇÃO: ST.PILLS PARA MODO DE LANÇAMENTO
        ...
        if "Provas" in modo_lancamento:
            ...
            if not t_sel or not at_sel:
                ...
                with col_fila:
                    with st.container(border=True):
                        ...
                        st.markdown("<br>", unsafe_allow_html=True)
                        if st.button("Ausências em Lote (Marcar Faltosos)", use_container_width=True, key=f"btn_flt_lote_{v}"):
                        ...
                        if st.session_state.get("show_faltas_lote", False):
                            with st.container(border=True):
                                ...
                                if st.button("Confirmar Ausências em Lote", type="primary", use_container_width=True, key=f"btn_conf_flt_lote_{v}"):
                                    if not faltosos:
                ...
                with col_mesa:
                    if alunos_alvo:
                        # 🚨 WORKSTATION DA MESA DE CORREÇÃO ISOLADA EM FRAGMENTO
                        @st.fragment
                        def renderizar_mesa_correcao_fragmento():
                            with st.container(border=True):
                                ...
                                for nome_a in alunos_alvo:
                                    ...
                                    if "(PEI N3)" in nec_aluno: st.caption("Status: 🔴 PEI NÍVEL 3 (QUALITATIVA)")
                                        ...
                                        st.caption("Status: 📝 PERFIL REGULAR / TÍPICO")

                            # 🚨 TRAVA ESTRITA DE PERFIL CLÍNICO (LEI Nº 5 DO PROTOCOLO)
                            primeira_nec = perfis_dupla[0][2] if perfis_dupla else ""
                            if "(PEI N3)" in primeira_nec or "NÍVEL 3" in primeira_nec or "NIVEL 3" in primeira_nec:
                                idx_lente_default = 3
                            elif "(PEI N2)" in primeira_nec or "NÍVEL 2" in primeira_nec or "NIVEL 2" in primeira_nec:
                                idx_lente_default = 2
                            elif "(PEI N1)" in primeira_nec or "NÍVEL 1" in primeira_nec or "NIVEL 1" in primeira_nec:
                                idx_lente_default = 1
                            elif tem_pei_na_dupla:
                                idx_lente_default = 1
                            else:
                                idx_lente_default = 0  # REGULAR FIXO EM 0 PARA ALUNOS TÍPICOS
                            ...
                            with st.container(border=True):
                                # 🚨 UI NOVA GERAÇÃO: ST.SEGMENTED_CONTROL PARA LENTE DE CORREÇÃO
                                lente_corr = st.segmented_control(
                                    "Lente de Correção (Auto-mapeada pelo Perfil Clínico):", 
                                    opcoes_lentes,
                            ...
                            if material_ref is not None:
                                ...
                                # 🚨 UI NOVA GERAÇÃO: ST.POPOVER PARA GABARITO BASE
                                ...
                                if modo_2a:
                                    ...
                                    if st.button("Salvar Avaliação Discursiva", type="primary", use_container_width=True, key=f"btn_save_2a_{v}"):
                                        ...
                                        for aluno_nome in alunos_alvo:
                                            match_al_disc = alunos_turma_df[alunos_turma_df['NOME_ALUNO'] == aluno_nome]
                                            if not match_al_disc.empty:
                                                id_al = db.limpar_id(match_al_disc.iloc[0]['ID'])
                                    ...
                                    # 🚨 UI NOVA GERAÇÃO: ST.PILLS PARA MÉTODO DE CORREÇÃO
                                    ...
                                    if c_m2.button("Ausência", use_container_width=True, key=f"btn_aus_single_{v}"):
                                        for aluno_nome in alunos_alvo:
                                            ...
                                            if not match_al_aus.empty:
                                                id_al = db.limpar_id(match_al_aus.iloc[0]['ID'])
                                                db.excluir_registro("DB_GABARITOS_ALUNOS", id_al)
                                                    ...
                                                    for aluno_nome in alunos_alvo:
                                                        ...
                                                        if not match_al.empty:
                                                            ...
                                                            db.salvar_no_banco("DB_GABARITOS_ALUNOS", [
                                                                datetime.now().strftime("%d/%m/%Y"), id_al, aluno_nome, t_sel, material_ref['TIPO_MATERIAL'], respostas_salvar, util.sosa_to_str(nota_f), link_foto_jpg
                                                            ])
                                        ...
                                        if st.button("Gravar Correção Manual", type="primary", use_container_width=True, key=f"btn_save_man_{v}"):
                                            ...
                                            for aluno_nome in alunos_alvo:
                                                ...
                                                if not match_al_man.empty:
                                                    id_al = db.limpar_id(match_al_man.iloc[0]['ID'])
                                                    db.excluir_registro("DB_GABARITOS_ALUNOS", id_al)
                                    ...
                                    if st.button("Salvar Avaliação PEI N3", type="primary", use_container_width=True, key=f"btn_save_pei3_{v}"):
                                        ...
                                        for aluno_nome in alunos_alvo:
                                            ...
                                            if not match_al_q.empty:
                                                id_al = db.limpar_id(match_al_q.iloc[0]['ID'])
                                                db.excluir_registro("DB_GABARITOS_ALUNOS", id_al)
            ...
            if t_sel_a and at_sel_a:
                ...
                df_notas_ed = st.data_editor(
                    ...
                    key=f"ed_at_{at_sel_a.replace(' ','_')}_{v}"
                )

                if st.button("Gravar Lote de Notas no Diário", type="primary", use_container_width=True, key=f"btn_save_trab_{v}"):
    ...
    # ==============================================================================
    # ABA 2: TRIBUNAL DE AUDITORIA (MESA DE ESPELHO SPLIT-SCREEN EM FRAGMENTO)
    # ==============================================================================
    with tab_auditoria:
        ...
        if t_sel_h and tr_sel_h:
            ...
            if av_alvo_h:
                ...
                if c_act1.button("⚖️ Atestados & Justificativas", use_container_width=True, key=f"btn_act_atest_{v}"): dialog_atestados_modal()
                if c_act2.button("👤 Perícia por Estudante", use_container_width=True, key=f"btn_act_peric_{v}"): dialog_pericia_modal()
                if c_act3.button("🚑 Digitação Manual (Lázaro)", use_container_width=True, key=f"btn_act_laz_{v}"): dialog_lazaro_modal()

                st.markdown("---")

                # ------------------------------------------------------------------
                # 🚀 BENTO GRID SPLIT-SCREEN ISOLADA EM FRAGMENTO
                # ------------------------------------------------------------------
                @st.fragment
    ...
    # ==============================================================================
    # ABA 3: RAIO-X PEDAGÓGICO (AUTÓPSIA POR ITEM)
    # ==============================================================================
    with tab_raiox:
        ...
        if not t_sel_r or not at_sel_r:
            ...
            if respostas_brutas.empty:
                ...
                if not cadernos_disponiveis:
                    ...
                    with st.container(border=True):
                        # 🚨 UI NOVA GERAÇÃO: ST.PILLS PARA CADERNOS
...
# ==============================================================================
    ...
    # 🚨 CONFIGURAÇÃO DE PESOS E REGRAS DA MÉDIA
    ...
    if not lista_turmas_notas:
        st.warning("⚠️ Nenhuma turma regular cadastrada no sistema.")
    else:
        # 🚨 1. INTELIGÊNCIA TEMPORAL AUTOMÁTICA (AUTO-DETECÇÃO DE TRIMESTRE)
        hoje_dt = date.today()
        if hoje_dt <= date(2026, 5, 22):
            idx_trim_default = 0 # I Trimestre
        elif hoje_dt <= date(2026, 9, 4):
            idx_trim_default = 1 # II Trimestre (Detectado para Agosto!)
        else:
            idx_trim_default = 2 # III Trimestre

        # MODAL DE CONFIGURAÇÃO DE PESOS
        @st.dialog("⚙️ Pesos & Regras da Média", width="medium")
        def dialog_config_pesos_modal():
            st.caption("Ajuste a composição da Média Final (A soma dos pesos deve dar 10.0).")
            c_p1, c_p2, c_p3 = st.columns(3)
            st.session_state.p_visto = c_p1.number_input("C1 (Vistos):", 0.0, 10.0, st.session_state.p_visto, step=0.5)
            st.session_state.p_teste = c_p2.number_input("C2 (Teste):", 0.0, 10.0, st.session_state.p_teste, step=0.5)
            st.session_state.p_prova = c_p3.number_input("C3 (Prova):", 0.0, 10.0, st.session_state.p_prova, step=0.5)
            
            st.markdown("---")
            st.session_state.regra_rec = st.selectbox(
                "⚖️ Regra da Recuperação Paralela:", 
            ...
            st.session_state.arredondar_prefeitura = st.toggle("⚖️ Arredondamento da Prefeitura (0,5 em 0,5)", value=st.session_state.arredondar_prefeitura)
            
            soma_p = st.session_state.p_visto + st.session_state.p_teste + st.session_state.p_prova
            if soma_p != 10.0:
                st.error(f"⚠️ A soma dos pesos ({soma_p:.1f}) deve ser exatamente 10.0.")
            else:
                if st.button("💾 Salvar Regras", type="primary", use_container_width=True):
                    st.rerun()

        # --- 2. BARRA SUPERIOR DE COMANDO BENTO GRID ---
        with st.container(border=True):
            c_f1, c_f2, c_f3, c_f4 = st.columns([1.5, 1.5, 1.5, 1])
            turma_sel = c_f1.selectbox("👥 Turma:", lista_turmas_notas, key=f"n_turma_{v}")
            
            opcoes_trimestres = ["I Trimestre", "II Trimestre", "III Trimestre"]
            trimestre_sel = c_f2.selectbox(
                "📅 Trimestre (Auto-Select):", 
                opcoes_trimestres, 
                index=idx_trim_default, 
                key=f"n_trim_{v}",
        ...
        if alunos_turma.empty:
            ...
            if not df_diario.empty:
                ...
                aulas_com_visto_count = df_d_trim[df_d_trim['VISTO_ATIVIDADE'].astype(str).str.upper() == "TRUE"]['DATA'].nunique()
            ...
            for _, alu in alunos_turma.iterrows():
                ...
                if not df_diag_turma.empty:
                    ...
                    if not scanned_prova.empty: 
                        ...
                        if "2ª" in av_nome or "2CHAMADA" in av_nome: origem_tag = "2ªC Chamada"
                        elif "TIPO" in av_nome: origem_tag = f"🧬 Var.{av_nome.split('TIPO')[-1].strip()}"
                        else: origem_tag = "⚡ Scanner"
            ...
            st.session_state.df_input_ref = df_input.copy()

            # 🚨 4. TERMÔMETRO DA TURMA (KPI BENTO CARDS NO TOPO)
            st.markdown("#### 📊 Termômetro da Turma (Auto-Sincronizado)")
            
            total_alunos = len(df_input)
            aprovados_count = len(df_input[df_input['MEDIA_FINAL'] >= 6.0])
            perc_aprovados = (aprovados_count / total_alunos) * 100 if total_alunos > 0 else 0
            uti_count = len(df_input[df_input['MEDIA_FINAL'] < 5.5])
            media_geral_turma = df_input['MEDIA_FINAL'].mean() if total_alunos > 0 else 0.0

            with st.container(border=True):
                k1, k2, k3, k4 = st.columns(4)
                k1.metric("📊 Média da Turma", f"{media_geral_turma:.1f}")
                k2.metric("🟢 Aprovados Direto", f"{aprovados_count} ({perc_aprovados:.0f}%)", f"{total_alunos} alunos")
                k3.metric("🚑 Risco / UTI (< 5.5)", uti_count, delta_color="inverse" if uti_count > 0 else "normal")
                k4.metric("📓 Aulas com Vistos", aulas_com_visto_count)
            ...
            # 🚨 6. MESA DE OPERAÇÕES EM FRAGMENTO
            @st.fragment
            def renderizar_mesa_notas_fragmento():
                tab_lancamento, tab_fechamento = st.tabs(["📝 Mesa de Lançamento (Auto-Sync)", "🖨️ Relatório de Fechamento"])
                ...
                with tab_lancamento:
                    ...
                    df_editado = st.data_editor(
                        ...
                        hide_index=True, use_container_width=True, key=f"editor_notas_frag_{v}"
                    )

                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("💾 SALVAR NOTAS E SINCRONIZAR BOLETIM", type="primary", use_container_width=True, key=f"btn_save_boletim_{v}"):
                ...
                with tab_fechamento:
                    ...
                    c_met1.caption(f"Status: ✅ APROVADOS DIRETO ({len(df_aprovados)})")
                    c_met2.caption(f"Status: 🟡 QUASE LÁ ({len(df_quase)})")
                    c_met3.caption(f"Status: 🔴 RECUPERAÇÃO ({len(df_rec)})")
                    
                    st.markdown("---")
                    
                    c_list1, c_list2 = st.columns(2)
                    with c_list1:
                        st.markdown("#### 🟡 Radar: Quase Lá (5.5 a 5.9)")
                    ...
                    with c_list2:
                        st.markdown("#### 🔴 UTI: Recuperação Paralela (< 5.5)")
                        if not df_rec.empty:
                    ...
                    st.markdown("---")
                    st.markdown("#### 🖨️ Fábrica de Etiquetas Word")
                    st.info("O sistema gerará um documento Word com retângulos formatados para colar na prova.")
                    
                    if st.button("🖨️ GERAR ETIQUETAS (DOCX)", use_container_width=True, type="primary", key=f"btn_etiq_docx_{v}"):
        ...
        def renderizar_dossie_bio_fragmento():
            ...
            with tabs[2]:
                ...
                def dialog_tribunal():
                    ...
                    av_contestada = st.selectbox("1️⃣ Selecione a Avaliação Questionada:", opcoes_av_tribunal, key=f"trib_av_{v}")
                    ...
                    if av_contestada:
                        ...
                        if not df_prova_trib.empty:
                            ...
                            q_contestada = st.selectbox("2️⃣ Selecione a Questão:", [f"Questão {i}" for i in range(1, qtd_questoes_trib + 1)], key=f"trib_q_{v}")
                            ...
                            if c_ver1.button("🔴 A Nota Fica (Gerar Defesa Pedagógica)", use_container_width=True, key=f"btn_def_trib_{v}"):
                            ...
                            if c_ver2.button("🟢 O Pai Tem Razão (Corrigir Nota)", use_container_width=True, key=f"btn_corr_trib_{v}"):
                            ...
                            if st.session_state.get("modo_correcao_tribunal", False):
                                with st.container(border=True):
                                    ...
                                    nova_letra = st.selectbox("Qual letra o aluno realmente marcou?", ["A", "B", "C", "D", "E"], index=["A", "B", "C", "D", "E"].index(letra_correta_trib) if letra_correta_trib in ["A", "B", "C", "D", "E"] else 0, key=f"sel_letra_trib_{v}")
                                    
                                    if st.button("💾 Confirmar Correção e Recalcular Média", type="primary", key=f"btn_conf_trib_{v}"):
...
# ==============================================================================
    ...
    if df_notas.empty:
        ...
        # --- 1. FILTRO DE TURMA E ATA DE CONSELHO ---
        lista_turmas_bol = []
        if not df_turmas.empty and 'ID_TURMA' in df_turmas.columns:
        ...
        with st.container(border=True):
            ...
            # 🚨 UI NOVA GERAÇÃO: ST.POPOVER PARA ATA DO CONSELHO DE CLASSE
        ...
        # 🚨 MAPA DO CONSELHO ISOLADO EM FRAGMENTO (NAVEGAÇÃO RÁPIDA)
        @st.fragment
        def renderizar_boletim_anual_fragmento():
            ...
            if df_t.empty:
                ...
                # CÁLCULO DA SITUAÇÃO ANUAL COM INTELIGÊNCIA TEMPORAL
                ...
                # --- 4. KPIs DE TOPO (TERMÔMETRO DA TURMA COM BADGES) ---
                ...
                risco_total = len(pivot[pivot['SITUAÇÃO'].isin(["🚨 REPROV. FALTA", "🟡 ALERTA (NOTA)", "🔴 RISCO CRÍTICO"])])
...
# ==============================================================================
    ...
    # ==============================================================================
    # ABA 1: COCKPIT DE REGÊNCIA (ISOLADO EM FRAGMENTO)
    # ==============================================================================
    with tab_cockpit:
            ...
            if not lista_turmas_segura:
                ...
                if chave_lista_hoje in st.session_state:
                    ...
                    if concluidos_arg and pendentes_arg:
                        st.warning(f"⚠️ **SESSÃO DE ARGUIÇÃO EM ANDAMENTO ({len(concluidos_arg)} avaliados / {len(pendentes_arg)} pendentes):** Alunos no quadro aguardando lançamento!")
                ...
                def dialog_roleta(t_roleta):
                    ...
                    if c_btn_sort.button("🎲 SORTEAR ESQUADRÃO", type="primary", use_container_width=True, key=f"btn_sort_{v}"):
                ...
                def dialog_dia_nao_letivo(t_foco):
                    ...
                    motivo_nl = st.text_input("Motivo:", placeholder="Ex: Paralisação Sindical", key=f"nl_mot_{v}")
                ...
                # 🚨 COCKPIT DE REGÊNCIA ISOLADO EM FRAGMENTO
                @st.fragment
                        ...
                        if not historico_turma_auditoria.empty:
                            ...
                            for i_a, (idx, row_aula) in enumerate(aulas_abertas.iterrows()):
                                with st.container(border=True):
                                    ...
                                    if c_btn_save.button("Salvar", key=f"aud_save_{i_a}_{v}", type="primary"):
                                    ...
                                    if c_btn_del.button("Apagar", key=f"aud_del_{i_a}_{v}"):
    ...
    # ==============================================================================
    # ABA 2: MÁQUINA DO TEMPO (ISOLADA EM FRAGMENTO)
    # ==============================================================================
    with tab_maquina:
        @st.fragment
        def renderizar_maquina_tempo_fragmento():
            ...
            if not lista_turmas_segura:
                ...
                if t_maq:
                    df_d_maq = df_diario[(df_diario['TURMA'] == t_maq) & (~df_diario['TAGS'].isin(["DIA NÃO LETIVO", "BONUS_CONSELHO", "SISTEMA_NOTA"]))] if not df_diario.empty else pd.DataFrame()
                    ...
                    if df_d_maq.empty:
                        ...
                        aula_info = df_registro_aulas[(df_registro_aulas['DATA'] == data_maq) & (df_registro_aulas['TURMA'] == t_maq)] if not df_registro_aulas.empty else pd.DataFrame()
                        ...
                        alunos_da_turma = df_alunos[df_alunos['TURMA'] == t_maq] if not df_alunos.empty else pd.DataFrame()
                        ...
                        def dialog_visto_excecao():
                            ...
                            if elegiveis.empty:
                                ...
                                aluno_alvo = st.selectbox("Estudante:", elegiveis['NOME_ALUNO'].tolist(), key=f"sel_al_visto_exc_{v}")
                                motivo_excecao = st.selectbox("Motivo da Justificativa:", ["Atestado Médico", "Luto", "Problema Familiar", "Atraso Justificado", "Entrega Tardia Autorizada"], key=f"sel_mot_visto_exc_{v}")
                                
                                if st.button("💾 Atribuir Visto e Salvar Justificativa", type="primary", use_container_width=True, key=f"btn_save_visto_exc_{v}"):
                                    id_aluno_alvo = db.limpar_id(elegiveis[elegiveis['NOME_ALUNO'] == aluno_alvo].iloc[0]['ID'])
                        ...
                        if st.button("⚖️ JUSTIFICAR AUSÊNCIA E ATRIBUIR VISTO", type="primary", use_container_width=True, key=f"btn_open_visto_exc_{v}"):
                            dialog_visto_excecao()

        renderizar_maquina_tempo_fragmento()

    # ==============================================================================
    # ABA 3: INTELIGÊNCIA DA TURMA E FEED SEMÂNTICO (ISOLADO EM FRAGMENTO)
    # ==============================================================================
    with tab_inteligencia:
        @st.fragment
        def renderizar_feed_semantico_fragmento():
            ...
            if t_rad:
                alunos_rad = df_alunos[df_alunos['TURMA'] == t_rad].copy() if not df_alunos.empty else pd.DataFrame()
                if alunos_rad.empty: st.info("Nenhum aluno cadastrado nesta turma.")
                    ...
                    df_d_rad = df_diario[df_diario['TURMA'] == t_rad].copy() if not df_diario.empty else pd.DataFrame()
                    ...
                    df_diag_rad = df_diagnosticos[df_diagnosticos['TURMA'] == t_rad].copy() if not df_diagnosticos.empty else pd.DataFrame()
                    ...
                    with col_uti:
                        ...
                        df_n_trim = df_notas[(df_notas['TURMA'] == t_rad) & (df_notas['TRIMESTRE'] == trim_rad)].copy() if not df_notas.empty else pd.DataFrame()
                    ...
                    if not df_d_rad.empty:
                        ...
                        if not obs_reais.empty:
                            ...
                            for idx, row_obs in ultimas_obs.iterrows():
                                id_alu_obs = db.limpar_id(row_obs['ID_ALUNO'])
                                ...
                                with st.container(border=True):
                                        ...
                                        with st.spinner("Atualizando..."):
                                            ...
                                            try:
                                                ...
                                                for i, row_d in enumerate(dados_diario):
                                                    if i > 0 and row_d[0] == data_obs and db.limpar_id(row_d[1]) == id_alu_obs and row_d[6].strip() == texto_obs.strip():
                                    ...
                                    if c_btn1.button("✅ Ciente", key=f"feed_ok_{idx}_{v}", use_container_width=True): processar_acao_feed("Ciente")
                                    if c_btn2.button("🧱 Def. Leitura", key=f"feed_dl_{idx}_{v}", use_container_width=True): processar_acao_feed("Defasagem Leitura", "DEFASAGEM LEITURA")
                                    if c_btn3.button("🧮 Def. Mat.", key=f"feed_dm_{idx}_{v}", use_container_width=True): processar_acao_feed("Defasagem Mat.", "DEFASAGEM MATEMÁTICA")
                                    if c_btn4.button("🟠 Suspeita PEI", key=f"feed_pei_{idx}_{v}", use_container_width=True): processar_acao_feed("Suspeita PEI", "PEI - PENDENTE")
                                    if c_btn5.button("🚀 Alta Perf.", key=f"feed_alta_{idx}_{v}", use_container_width=True): processar_acao_feed("Alta Performance", "ALTA PERFORMANCE")
                        else: st.success("🎉 Nenhuma observação pendente.")
                    else: st.info("Sem registros no Diário.")

        renderizar_feed_semantico_fragmento()

    # ==============================================================================
    # ABA 4: SECRETARIA, MATRÍCULAS & CALENDÁRIO (COM ST.SEGMENTED_CONTROL E ST.PILLS)
    # ==============================================================================
    with tab_secretaria:
        st.markdown("### ⚙️ Secretaria, Matrículas & Calendário")
        
        # 🚨 UI NOVA GERAÇÃO: ST.SEGMENTED_CONTROL PARA SUB-ABAS DA SECRETARIA
        ...
        if modo_sec == "🏗️ Criar Turmas/Horários":
            # 🚨 UI NOVA GERAÇÃO: ST.PILLS PARA TIPO DE CADASTRO
