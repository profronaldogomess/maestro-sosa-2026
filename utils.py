...
def converter_latex_para_texto_word(texto):
    """
    SOSA V2026 - CONVERSOR MATEMÁTICO PROFMAT PARA WORD (.DOCX):
    Higieniza contra caracteres nulos XML e converte comandos LaTeX geométricos
    em símbolos Unicode limpos e compatíveis com a impressão física.
    """
    if not texto or not isinstance(texto, str): return ""
    t = sanitizar_xml_str(texto)
    
    # Ajustes monetários e porcentagens
    t = t.replace(r'R\$', 'R$').replace(r'R \$', 'R$')
    t = t.replace(r'\$', '$').replace(r'\%', '%')
    
    # Mapeamento de Símbolos Geométricos PROFMAT para Unicode Oficial
    t = re.sub(r'\\parallel\b', '∥', t)
    t = re.sub(r'\\perp\b', '⊥', t)
    t = re.sub(r'\\cong\b', '≅', t)
    t = re.sub(r'\\sim\b', '∼', t)
    t = re.sub(r'\\Delta\b|\\triangle\b', 'Δ', t)
    t = re.sub(r'\\angle\b', '∠', t)
    t = re.sub(r'\\circ\b|\\degree\b', '°', t)
    t = re.sub(r'\\cdot\b', '·', t)
    t = re.sub(r'\\times\b', '×', t)
    t = re.sub(r'\\div\b', '÷', t)
    t = re.sub(r'\\pm\b', '±', t)
    ...
    # Tratamento de Segmentos de Reta: \overline{AB} -> AB com macron ou formato legível
    t = re.sub(r'\\overline\{([^}]+)\}', r'segmento \1', t)
    t = re.sub(r'\\vec\{([^}]+)\}', r'semirreta \1', t)
