...
def gerar_docx_prova_v25(titulo_doc, conteudo_ia, info):
    ...
    # Criamos o stream logo no início para garantir que ele exista
    file_stream = io.BytesIO()
    ...
    try:
        ...
        # Numeração manual para evitar erro de estilo 'List Number'
        for idx, linha in enumerate(txt_orient.split('\n'), 1):
            if linha.strip():
                texto_limpo = re.sub(r'^\d+[\.\s\-]*', '', linha.strip())
                p_o = c_orient.add_paragraph()
                p_o.paragraph_format.left_indent = Inches(0.2)
                p_o.add_run(f"{idx}. {texto_limpo}").font.size = Pt(9)
        ...
        while i < len(partes):
            ...
            if marcador: final_q.append(f"{marcador} {texto}")
            i += 2
        ...
        # Se houver erro, gera um documento de erro para não retornar None
        error_doc = Document()
